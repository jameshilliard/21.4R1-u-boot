...
extern int32_t srxsme_get_next_bootdev(uint8_t *bootdev, int32_t update);
#endif  /* JUNOS end */
...
static int API_dev_enum(va_list ap)
{
	...
	if (di->cookie == NULL) {
		...
		if (IS_PLATFORM_SRX550(gd->board_desc.board_type)) {
			/*gd->jt[XF_disk_scan]();*/
			disk_export_scan(-1); /* this will call usb_disk_scan */
			disk_export_scan(0);  /* this will call srxle_cf_enable */
			if (boot_record_updated == 0) {
				uint8_t bootdev[32];
		srxsme_get_next_bootdev(bootdev, 1);
...
#ifdef CONFIG_JSRXNLE /* JUNOS begin */
    /*
     * the veriexec loader command 'devinfo' will re-enumberate the devies,
     * which will clear the DEV_STA_OPEN flag, it lead to the 'ls' command
     * not working.
     * Since there is no multi user for the loader, we keep the storage device
     * state OPEN to fix the problem.
     */
    else {
        int err = 0;
...
#ifndef CONFIG_JSRXNLE /* JUNOS begin */
...
extern uchar (*env_get_char)(int); /* JUNOS */
