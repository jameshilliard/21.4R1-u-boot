         ...
         ((gd->board_type == I2C_ID_ACX2100)? FORTIUS_GPLUS_##bit :  \
          ((gd->board_type == I2C_ID_ACX4000)? FORTIUS_M_##bit :  \
           ((gd->board_type >= I2C_ID_ACX500_O_POE_DC) &&          \
             (gd->board_type <= I2C_ID_ACX500_I_AC)) ? KOTINOS_##bit : \
			            bit)))
        ...
        ((gd->board_type == I2C_ID_ACX1000)? (val & 1<<FORTIUS_F_##bit) :   \
         ((gd->board_type == I2C_ID_ACX2100)? (val & 1<<FORTIUS_GPLUS_##bit) : \
          ((gd->board_type == I2C_ID_ACX4000)? (val & 1<<FORTIUS_M_##bit) : \
           ((gd->board_type >= I2C_ID_ACX500_O_POE_DC) &&          \
	    (gd->board_type <= I2C_ID_ACX500_I_AC)) ? (val & 1<<KOTINOS_##bit) : \
	    (val & 1<<bit))))
...
#define ACX_VARIANT_GET_VAL(reg)                                \
	...
	((gd->board_type >= I2C_ID_ACX500_O_POE_DC) &&          \
	(gd->board_type <= I2C_ID_ACX500_I_AC)) ? syspld_kotinos->re.reg :\
	    syspld_g->re.reg)))
 ...
 */
uint8_t syspld_set_gcr(uint8_t gcr)
{
	uint8_t old_gcr;
 ...
 * returns 0 otherwise
 */
int syspld_is_glass_safe_open(void)
{
	uint8_t	gcr;
 ...
 */
int syspld_close_glass_safe(void)
{
	uint8_t	gcr;
 ...
 */
int syspld_nor_flash_wp_set(uint8_t wp)
{
	uint8_t gcr;
 ...
 *	0 - If not.
 */
int syspld_tpm_pp_control(int set)
{
	uint8_t gcr;
 ...
 *	0 - if not.
 */
int syspld_cpld_update(int disable)
{
	uint8_t gcr;
 ...
 */
int syspld_push_button_detect(void)
{
	uint8_t misc;
 ...
 */
void syspld_assert_sreset(void)
{
	uint8_t val;
 ...
 */
int syspld_cpu_only_reset(void)
{
	uint8_t val;
 ...
 *	0 - if not
 */
int syspld_uart_rx_control(int enable)
{
	uint8_t uart_ctl;
 ...
 *	0 - if not
 */
int syspld_uart_tx_control(int enable)
{
	uint8_t uart_ctl;
 ...
 *	0 - if it is disabled
 */
int syspld_uart_get_rx_status(void)
{
	uint8_t uart_ctl;
 ...
 *	0 - if it is disabled
 */
int syspld_uart_get_tx_status(void)
{
	uint8_t uart_ctl;
 ...
 *
 */
void syspld_set_lpc_addr_byte1(uint8_t addr_val)
{
 ...
 *
 */
void syspld_set_lpc_addr_byte2(uint8_t addr_val)
{
 ...
 *
 */
void syspld_start_lpc_mem_read_trans(void)
{
    uint8_t lpc_csr_reg_val;
 ...
 *
 */
void syspld_start_lpc_mem_write_trans(void)
{
    uint8_t lpc_csr_reg_val;
 ...
 *  02 - Error
 */
int syspld_get_trans_result(void)
{
    uint8_t lpc_csr_reg_val;
 ...
 *
 */
uint8_t syspld_get_lpc_din(void)
{
    uint8_t tpm_din_val;
 ...
 *
 */
void syspld_set_lpc_dout(uint8_t data_out)
{
...
static void syspld_reset_ddr(uint8_t clear_bit)
{
    uint8_t reg_value;
...
static void syspld_reset_hw_ic_monitor(uint8_t clear_bit)
{
    uint8_t reg_value;
    uint8_t ext_device_reset_cntrl_reg = 0x14;
