 ...
 * $Id$
 *
 * i2c_cdev.c -- i2c driver specific  for iCRT
 *
		...
		signed char *temp)
{
    ...
    /*
     * If MULTIPLEXER_PRESENT & already enabled channels
     * disable channels
     */
    if (i2c_dev->has_multiplexer & MULTIPLEXER_PRESENT) {
	multiplexer_control_reg = 0;
	...
	/*
	 * Add -64 to get the range of -64 to +191 Degree C
	 * so 255 = 191 & 0 = -64
	 * and range is from -64 to +191.
	 */
        *temp = (data + MAX_6581_EXTD_TEMP_MIN);
	     ...
	     /*
	      * If Sign bit is set, return negative value
	      * by substracting the 0xFF
	      * for e.g - 0x82 - 0xFF = 83
	      * = -125
	      */
             if (data & 0x80) {
                 *temp = (data - 0xFF);
