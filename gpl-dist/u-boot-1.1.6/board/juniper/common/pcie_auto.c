...
int pcie_auto_config_device(struct pcie_controller *hose, pcie_dev_t dev)
{
	...
	if(pcie_mode == 7) /* Host(RC) */
	{
		 ...
		 */
#if defined(CONFIG_EX82XX) || defined(CONFIG_EX45XX)
		n = pcie_hose_scan_bus(hose, hose->current_busno);
#else
		pcie_hose_read_config_byte(hose, dev, 0x404, &link);
		if (link == 0x16)
			n = pcie_hose_scan_bus(hose, hose->current_busno);
#endif
			  ...
			  struct pcie_region *io)
{
	...
	for (bar = PCIE_BASE_ADDRESS_0; bar <= PCIE_BASE_ADDRESS_0 + (bars_num*4); bar += 4) {
...
#if defined(CONFIG_EX45XX)
		/* skip BAR0 on Lion and 6082 for EX4500 */
		pcie_hose_read_config_word(hose, dev, PCIE_VENDOR_ID, &vendor_id);

		if ((bar == PCIE_BASE_ADDRESS_0) && (vendor_id == 0x11ab)) {
			continue;
		}
#endif
		...
		if (bar_response & PCIE_BASE_ADDRESS_SPACE) {
            ...
            pcie_auto_region_align(io, 0x100000);
#endif
			...
			if ( (bar_response & PCIE_BASE_ADDRESS_MEM_TYPE_MASK) ==
				...
				found_mem64 = 1;
#if defined(CONFIG_EX82XX) || defined(CONFIG_EX45XX)
            pcie_auto_region_align(mem, 0x100000);
#endif
