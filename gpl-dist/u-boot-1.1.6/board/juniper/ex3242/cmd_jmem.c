...
void print_error(int step, UT_SIZE addr, UT_SIZE read, UT_SIZE expected)
{
    ...
    printf ("Mismatch pattern: %08lX -> ",mismatch);
...
int do_malgo(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    /*
     * Disable D cache for the ARM based platforms before the memory test
     */
    disableDcache();
#endif
    if (malgo_init == 0) {
    ...
    if (errAddr)
        ...
        printf("Memory algorithm test: start 0x%x end 0x%x pattern %s "
               ...
               errAddr, argv[7]);
#else
        printf("Memory algorithm test: start 0x%x end 0x%x pattern 0x%08x "
...
#if defined(CONFIG_EX2200) || defined(CONFIG_EX3300) || defined(CONFIG_RPS200)
                /*
                 * Enable D cache for the ARM based platforms after the memory test
                 */
                enableDcache();
#endif
                return 1;
...
#if defined(CONFIG_EX2200) || defined(CONFIG_EX3300) || defined(CONFIG_RPS200)
		/*
		 * For ARM based Ex2200, Ex3300 and RPS200  platforms, disable cache
		 * before memory test, and enable it afterwards.
		 */
		disableDcache();
#endif
		if (ctrlc()) {
...
#if defined(CONFIG_EX2200) || defined(CONFIG_EX3300) || defined(CONFIG_RPS200)
		/*
		 * Enable cache for memory tests that write to the entire memory.
		 */
		enableDcache();
#endif
		/*
