...
#define IRQ1_POLARITY    1  /* active high */
#define IRQ1_SENSE         0   /* edge sensitive */
#define IRQ1_PRIORITY    15  /* 0-15 (highest) */

void epld_watchdog_init(void)
{
       ...
       temp &= 0xff000000;
       temp += (IRQ1_POLARITY<<23) + (IRQ1_SENSE<<22) + (IRQ1_PRIORITY<<16) + IRQ1_INT_VEC;
       ccsr_pic_ptr->eivpr1 = temp;
/*       ccsr_pic_ptr->eidr1 = 0x1; */  /* default */
       temp &= 0x7fffffff;  /* enable IRQ1 */
       ccsr_pic_ptr->eivpr1 = temp;

}

void epld_watchdog_reset (void)
{
    epld_reg_write(EPLD_WATCHDOG_SW_UPDATE, 0x1);
}

int epld_watchdog_disable (void)
{
    return epld_reg_write(EPLD_WATCHDOG_ENABLE, 0x0);
}

int epld_watchdog_enable (void)
{
    return epld_reg_write(EPLD_WATCHDOG_ENABLE, 0x1);
}

void epld_watchdog_dump (void)
{
    uint16_t tmp;
