...
#define EX_CPU_WATCHDOG_INIT(x) 	cpu_watchdog_init(x)
#define EX_CPU_WATCHDOG_ENABLE()	cpu_watchdog_enable()
#define EX_CPU_WATCHDOG_DISABLE()	cpu_watchdog_disable()
#define EX_CPU_WATCHDOG_RESET() 	cpu_watchdog_reset()
...
#define EX_CPU_WDT_MASK			0x3f /* Max timer val for ppc e500 based platforms */
...
ex_get_next_bootdev (char *bootdev)
{
    ...
    if (str != NULL) {
	unattended = simple_strtoul(str, NULL, 10);
    } else {
	unattended = 0;
    }
...
#if defined(CONFIG_EX2200)
    if (btsq_disabled || unattended) {
        /*
         * boot sequencing is disabled or unattended mode is set,
	 * don't mess up with the * natural boot sequence 
         */
#else 
    if (btsq_disabled) {
        /*
	...
	if (btsq_disabled)
	    BSDEBUG("bootsequencing is disabled\n");
	if (unattended)
	    BSDEBUG("unattended mode is set\n");
#endif
...
#ifdef CONFIG_EX82XX
    /* 
     * On 82XX_LCPU, we do not want to mess up watchdog as bootsequencing
