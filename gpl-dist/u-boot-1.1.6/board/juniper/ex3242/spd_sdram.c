...
extern int version_minor(void);
extern int mem_ecc(void);
...
spd_sdram(void)
{
	...
	unsigned int accu_density = 0;
       ...
       /*
        * enable I2C DDR channel
        */
       i2c_reg_write(CFG_I2C_CTRL_1_SW_ADDR, 0, CFG_I2C_DDR_CHAN);
       ...
       for (dimm_num = 0; dimm_num < num_dimm_banks; dimm_num++) {
	    ...
	    if (!i2c_read(i2c_dimm_addr[dimm_num], 0, 1, (uchar *) &spd[dimm_num], sizeof(spd))) {
	        debug("DDR: DIMM[%d] is not populated = %d\n", dimm_num);
               continue;
           }
	    ...
	    if (spd[dimm_num].ncol_addr < 8 || spd[dimm_num].ncol_addr > 11) {
		    printf("DDR: DIMM[%d] unsupported number of Column Addr lines: %d.\n",
		           dimm_num, spd[dimm_num].ncol_addr);
		    continue;
	    }
	    ...
	    debug("DDR: DIMM[%d] number of ranks = %d\n", dimm_num, n_ranks[dimm_num]);

	    /*
	     * Determine the size of each Rank in bytes.
	     */
	    rank_density[dimm_num] = compute_banksize(spd[dimm_num].mem_type, spd[dimm_num].row_dens);

       }

       /*
        * disable I2C DDR channel
        */
       i2c_reg_write(CFG_I2C_CTRL_1_SW_ADDR, 0, 0);
	...
	if (spd[first_dimm].mem_type == SPD_MEMTYPE_DDR2) {
       ...
       for (dimm_num = 0; dimm_num < num_dimm_banks; dimm_num++) {
	    ...
	    if ( dimm_populated[dimm_num] && (cs_index + n_ranks[dimm_num] > CFG_DDR_CS)) {
		 printf("DDR: DIMM[%d] needs additional %d chip selects which is %d over maximum (%d) supported.\n",
		           dimm_num, n_ranks[first_dimm], cs_index + n_ranks[dimm_num] -CFG_DDR_CS, CFG_DDR_CS);
		 break;
	    }
		    ...
		    *cs_bnds = ( (accu_density >> 8)
				  | (((accu_density + rank_density[dimm_num]) >> 24)  - 1) );
		    *cs_config = ( 1<<31
				    | (odt_rd_cfg << 20)
				    | (odt_wr_cfg << 16)
	...
	if (spd[first_dimm].mem_type == SPD_MEMTYPE_DDR) {
		...
		trfc = (((spd[first_dimm].trctrfc_ext & 0x1) * 256) + spd[first_dimm].trfc) * 1000
	 ...
	 */
	if (spd[first_dimm].mem_type == SPD_MEMTYPE_DDR && spd[first_dimm].mod_attr == 0x26) {
...
spd_sdram(void)
{
	...
	volatile ccsr_ddr_t *ddr= &immap->im_ddr;
       int bid = board_id();
       uint32_t sdram_cfg = 0, msize = mem_size();
       sys_info_t sysinfo;
...
//       svr = get_svr();
	...
	ddr->cs0_config = (msize == 1024) ? 
        (CFG_DDR_CS0_CONFIG | CFG_DDR_3BIT_BANKS_CONFIG) : CFG_DDR_CS0_CONFIG;
	ddr->ext_refrec = CFG_DDR_REFREC;
	...
	ddr->sdram_mode_2 = CFG_DDR_MODE_2;
       if (sysinfo.freqSystemBus == 300000000)
	    ddr->sdram_interval = CFG_DDR_INTERVAL_CCB_300;
       else
...
#if defined (CONFIG_DDR_ECC)
      /* production Espresso will not support ECC.  The initial Espresso
           will still support ECC */
       if (!mem_ecc())
           /* No ECC support */
	    ddr->sdram_cfg = CFG_DDR_CFG;
       else
	    /* Enable ECC checking */
...
void
ddr_enable_ecc(unsigned int dram_size, int enable)
{
	uint *p = 0;
	uint i = 0;
	volatile immap_t *immap = (immap_t *)CFG_CCSRBAR;
