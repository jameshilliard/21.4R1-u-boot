...
struct pcie_controller pcie_hose[2];  /* 2 PEX for EX3300 */
...
int pcie_hose_scan_bus(struct pcie_controller *hose, int bus)
{
    ...
    for (dev =  PCIE_BDF(bus,0,0);
        ...
        vendor = 0;
        header_type = 0;
        /* Not sure this is compiler bug or timing issue.  Without insert
         *  a delay or printf here, it will get into a infinite loop.  The Lauterbach
         *  ICE does not stop at break point in this routine. 
         */
        udelay(1000);
        ...
        if (vendor != 0xffff && vendor != 0x0000) {
            ...
            pcie_auto_config_device(hose, dev);
        }
    }
...
int pcie_init( void )
{
    ...
    memset(pcie_hose, 0, 2 * sizeof(struct pcie_controller));

    pcie_info.pcie_mem_base = 0x40000000;
    pcie_info.pcie_mem_size = 0x8000000;
    pcie_arm_init(&pcie_hose[0], &pcie_info, 0);  /* pex0.1 */
    pcie_info.pcie_mem_base = 0x48000000;
    pcie_info.pcie_mem_size = 0x8000000;
    pcie_arm_init(&pcie_hose[1], &pcie_info, 4);  /* pex1.1 */
