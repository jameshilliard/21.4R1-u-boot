...
void 
pcie_auto_region_init (struct pcie_region* res)
{
	res->bus_lower = res->bus_start;
}
void 
pcie_auto_region_align (struct pcie_region *res, unsigned  long size)
{
	res->bus_lower = ((res->bus_lower - 1) | (size - 1)) + 1;
}

int 
pcie_auto_region_allocate (struct pcie_region* res, unsigned int size, unsigned long long *bar)
{
 ...
 *******************************************************************************/
void 
pcie_auto_config_init (struct pcie_controller *hose)
{
	int i;
 ...
 */
int 
pcie_auto_config_device (struct pcie_controller *hose, pcie_dev_t dev)
{
	...
	/* EX3300 bus 0/1 dev 0/1 */
	if ((sub_bus > hose->last_busno) || (PCIE_DEV(dev) > 1)) {
	...
	mvPexModeGet(mvPciRealIfNumGet(hose->last_busno * 4),&pcie_mode);
	...
	if(pcie_mode == PCI_IF_MODE_HOST) /* Host(RC) */
	{
		...
		if (sub_bus == 0)
			...
			link = MV_REG_READ(PEX_STATUS_REG(4)) & 0x1;
		else
			link = 1;
     	       /*
...
void 
pcie_auto_setup_device (struct pcie_controller *hose,
			  ...
			  struct pcie_region *io)
{
	...
	for (bar = PCIE_BASE_ADDRESS_0; bar <= PCIE_BASE_ADDRESS_0 + (bars_num*4); bar += 4) {
		...
		if (bar_response & PCIE_BASE_ADDRESS_SPACE) {
			...
			pcie_hose_write_config_dword(hose, dev, bar, bar_value);
			bar_nr++;
			continue;
		} else {
...
static void 
pcie_auto_prescan_setup_bridge (struct pcie_controller *hose,
...
static void 
pcie_auto_postscan_setup_bridge (struct pcie_controller *hose,
					  pcie_dev_t dev, int sub_bus)
{
	struct pcie_region *pcie_mem = hose->pcie_mem;
