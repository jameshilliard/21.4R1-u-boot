...
static char board_name [1][LCD_LINE_LENGTH+1] = 
 { 
    "EX4500-40F      "
};
...
lcd_write (LcdDataType dtype, uint8_t data)
{
    ...
    if (lcd_wait_till_not_busy(dtype ? EPLD_LCD_DATA_WRITE : 
                               EPLD_LCD_INST_WRITE)) {
#if (EPLD_LCD_4BIT_MODE)
        ...
        /* Delay of 100 micro seconds to resolve LCD char curruption */
        udelay(100);
...
lcd_heartbeat (int unit)
{
    ...
    static int index[CFG_LCD_NUM] = {0, 0};
...
lcd_backlight_control (int unit, LcdBacklight state)
{
    ...
    if (!epld_reg_read(EPLD_LCD_BACKLIGHT_REG, &tmp)) {
	 debug ("Unable to read EPLD register address 0x%x.\n",
            EPLD_LCD_BACKLIGHT_REG*2);
        return 0;
    }

    tmp &= ~(1 << unit);
    tmp |= (state ? (1 << unit) : 0);
    if (!epld_reg_write(EPLD_LCD_BACKLIGHT_REG, tmp)) {
	 debug ("Unable to write EPLD register address 0x%x[0x%x].\n",
            EPLD_LCD_BACKLIGHT_REG*2, tmp);
        return 0;
    }
