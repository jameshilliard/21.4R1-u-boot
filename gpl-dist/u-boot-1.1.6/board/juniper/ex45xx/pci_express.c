...
struct pcie_controller pcie_hose[1];
#endif

#ifdef CONFIG_MPC8544
#define MAX_PCIE_CONTROLLER 3
static unsigned long pcie_memory_base[MAX_PCIE_CONTROLLER] = 
{CFG_PCIE1_MEM_BASE,
  CFG_PCIE2_MEM_BASE,
  CFG_PCIE3_MEM_BASE
};
static unsigned long pcie_memory_size[MAX_PCIE_CONTROLLER] = 
{CFG_PCIE1_MEM_SIZE,
  CFG_PCIE2_MEM_SIZE,
  CFG_PCIE3_MEM_SIZE
};
static uint pcie_controller_offset[MAX_PCIE_CONTROLLER] = 
{ CFG_PCIE1_OFFSET, 
   CFG_PCIE2_OFFSET, 
   CFG_PCIE3_OFFSET
};

struct pcie_controller pcie_hose[MAX_PCIE_CONTROLLER];
#endif
...
int pcie_init( void )
{
    ...
    memset(pcie_hose, 0, sizeof(struct pcie_controller));
    pcie_hose_head = NULL;
    pcie_mpc85xx_init(&pcie_hose[0], &pcie_info);
#endif

#ifdef CONFIG_MPC8544
    int i;
    extern void pcie_mpc85xx_init(struct pcie_controller *hose, PARAM_PCIE *pcie_info, uint pcie_offset);

    for (i=0; i<MAX_PCIE_CONTROLLER; i++) {
        pcie_info.pcie_mem_base = (unsigned long long)pcie_memory_base[i];
        pcie_info.pcie_mem_size = (unsigned long long)pcie_memory_size[i];
        pcie_mpc85xx_init(&pcie_hose[i], &pcie_info, pcie_controller_offset[i]);
    }
#endif
