...
{
	gd->mpc_i2c = (controller == CFG_I2C_CTRL_2) ?
				  ((void*)(CFG_CCSRBAR + CFG_I2C2_OFFSET)) :
				  ((void*)(CFG_CCSRBAR + CFG_I2C_OFFSET));
}
...
void i2c_init(int speed, int slaveadd)
{
	...
	/* set clock frequency to 100KHz.
	   [ SCL frequency = (CCB clock/2)/(frequency divider) ] */
	writeb(0x2f, &I2C->fdr);
...
int i2c_raw_read(u8 dev, u8 *data, int length)
{
	int i = -1;
	...
	if (i2c_wait4bus() >= 0) {
		...
		debug("i2c_raw_read: Timed out \n");
	}

	if (length && i2c_write_addr(dev, I2C_READ, 0) != 0) {
		i = __i2c_read(data, length);
	} else   {
		debug("i2c_raw_read: length %d or write address failed \n", length);
	}
...
int i2c_dev_probe(u8 dev)
{
	...
	if (length && i2c_write_addr(dev, I2C_WRITE, 0) != 0) {
		/*Got ack. Device present*/
	} else   {
		debug("i2c_dev_probe: write address to device %02X failed \n", dev);
		return -1;
	}
...
int i2c_raw_write(u8 dev, u8 *data, int length)
{
	int i = -1;
...
int i2c_probe_caffeine(uint i2c_ctrl, uchar sw_addr, uchar ch_index, uchar chip)
{
	int tmp, ret = 1;
	unsigned char ch;
