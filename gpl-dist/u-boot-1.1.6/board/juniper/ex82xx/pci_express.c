...
#ifdef DEBUG
unsigned int pcie_dbg_print = 1;
#else
unsigned int pcie_dbg_print = 0;
#endif
...
#define PCIE_READ_VIA_DWORD_OP(size, type, off_mask)			   \
	int pcie_hose_read_config_ ## size ## _via_dword(			   \
	...
	{															   \
		unsigned int val32;										   \
																   \
		if (pcie_hose_read_config_dword(hose, dev, offset & 0xffc, \
										&val32) < 0) {			   \
			return -1; }										   \
																   \
...
#define PCIE_WRITE_VIA_DWORD_OP(size, type, off_mask, val_mask)		\
	int pcie_hose_write_config_ ## size ## _via_dword(				\
		struct pcie_controller*hose,								\
		pcie_dev_t dev,												\
		int offset,													\
		type val)													\
	{																\
		...
		val32 = (val32 & ~mask) | ldata;							\
																	\
		if (pcie_hose_write_config_dword(hose, dev, offset & 0xffc,	\
										 val32) < 0) {				\
			return -1; }											\
																	\
		return 0;													\
...
int pcie_hose_get_max_bus(int hose_id)
{
	struct pcie_controller*hose;
	int i = 0;
	...
	for (hose = pcie_hose_head; hose; hose = hose->next) {
		for (bus = hose->first_busno; bus <= hose->last_busno; bus++) {
			for (dev = 0; dev < PCIE_MAX_PCIE_DEVICES; dev++) {
				...
				for (function = 0;
					 function < PCIE_MAX_PCIE_FUNCTIONS;
					 function++) {
...
pcie_dev_t pcie_find_device(unsigned int vendor,
							unsigned int device,
							int index)
{
...
int pcie_hose_scan_bus(struct pcie_controller*hose, int bus)
{
	...
	for (dev =  PCIE_BDF(bus, 0, 0);
		 dev <
		 PCIE_BDF(bus, PCIE_MAX_PCIE_DEVICES - 1, PCIE_MAX_PCIE_FUNCTIONS - 1);
		 dev += PCIE_BDF(0, 0, 1)) {
		/* If this is not a multi-function device, we skip the rest. */
		...
		if (vendor != 0xffff && vendor != 0x0000) {
			if (pcie_dbg_print) {
				printf(
					"PCI Express Scan: Found Bus %d, Device %d, Function %d : vendor is %x\n",
					PCIE_BUS(dev),
					PCIE_DEV(dev),
					PCIE_FUNC(dev),
					vendor);
