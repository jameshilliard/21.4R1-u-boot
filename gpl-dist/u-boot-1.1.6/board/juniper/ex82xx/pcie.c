...
extern void pcie_setup_indirect(struct pcie_controller*hose,
...
#define PCIE_TRANSLATION_ADDR_BASE    0x04000000                /* TARGET's DDR MEMORY BASE for EP PCIE memory space */
		...
		/* init inbound window 1MB DDR memory */
		pcie->itar2  =
			((PCIE_TRANSLATION_ADDR_BASE + 0x100000) >> 12) & 0x000fffff;
		pcie->iwbar2 = 0x00000000;
		pcie->iwar2  = 0x80f55013;       /* Enable,  Non Prefetch, Local Mem,
										  * Snoop R/W, 1MB */
		/* init inbound window 1MB DDR memory */
		pcie->itar3  =
			((PCIE_TRANSLATION_ADDR_BASE + 0x200000) >> 12) & 0x000fffff;
		...
		pcie->otar1   =
			(unsigned int)((pcie_info->pcie_mem_base >> 12) & 0x000fffff);
		...
		pcie->owar1 =
			(POWAR_EN | POWAR_MEM_READ | POWAR_MEM_WRITE | POWAR_MEM_1G);
		...
		for (i = 0x00; i < 0x1000; i = i + 4) {
			pcie_read_config_dword(PCIE_BDF(0, 0, 0), i, &temp);
