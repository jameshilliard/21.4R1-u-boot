...
int tsec_initialize(bd_t * bis, int index, char *devname)
{
	...
	if (priv->regs->ecntrl & ECNTRL_SGMII_MODE) {
		debug("TSEC SGMII Interface Index = %d, phyaddr = %d\n", index, priv->phyaddr);
	}
#endif
	...
	priv->regs->tbipa = CFG_TBIPA_VALUE + index;
	...
	/* phyaddr must be unique */
	priv_tbi->phyaddr = CFG_TBIPA_VALUE + index;
	priv_tbi->flags = 0;
	...
	priv->regs->maccfg1 &= ~(MACCFG1_SOFT_RESET);
	debug("soft reset mac\n");

	/* Try to initialize PHY here, and return */
	/*
	 * Only eTSEC3 (fxp0) supports PHY.
	 * Skip PHY initialization for other eTSEC controllers.
	 */
	if (!strcmp(dev->name, CONFIG_MPC85XX_TSEC3_NAME)) {
		return init_phy(dev);
	} else {
	    debug("No PHY supported for %s\n", dev->name);
	    return 1;
	}
}
...
struct phy_info phy_info_M88E1112S = {
	...
	(struct phy_cmd[]){	/* config *//* sgmii copper */
			   ...
			   {MIIM_88E1112_PHY_LED_POLARITY, MIIM_88E1112_PHY_LED_POLARITY_DEFAULT, NULL},  /* link LED Polarity */
			   {MIIM_88E1112_PHY_PAGE, 0x0, NULL},  /* page 0 */
			   {miim_end,}
			   },
	(struct phy_cmd[]){	/* startup */
