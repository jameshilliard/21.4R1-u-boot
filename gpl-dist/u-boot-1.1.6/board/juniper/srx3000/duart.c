...
void
D16C752B_init (DUART_16C752B_t com_port, int baud_divisor)
{
	com_port->ier = 0x00;
#ifdef CONFIG_OMAP
...
void 
D16C752B_init_internal (DUART_16C752B_t com_port, int baud_divisor)
{
	com_port->ier = 0x00;
#ifdef CONFIG_OMAP
	com_port->mdr1 = 0x7;   /* mode select reset TL16C750*/
...
void
D16C752B_reinit (DUART_16C752B_t com_port, int baud_divisor)
{
	com_port->ier = 0x00;
...
void
D16C752B_setbrg (const int port)
{
	int clock_divisor;
...
void
D16C752B_duart_putc (DUART_16C752B_t com_port, char c)
{
...
char
D16C752B_duart_getc (DUART_16C752B_t com_port)
{
...
void
duart_putc (unsigned int com_port, char c)
{
	if(c == '\n') {
...
char
duart_getc (unsigned int com_port)
{
	return D16C752B_duart_getc(serial_ports[com_port-1]);
}

void
duart_puts (const char *s)
{
	while(*s) 
