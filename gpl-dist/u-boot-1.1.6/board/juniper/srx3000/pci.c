...
#ifdef CONFIG_RMI
extern uint32_t rmi_link_state[];
extern int rmi_change_class;
extern int rmi_config_bus0;
#endif

#ifndef CONFIG_IXP425
pci_dev_t 
pci_find_devices (struct pci_device_id *ids, int index)
{
	struct pci_controller * hose;
	u16 vendor, device;
	u8 header_type;
...
static pci_vend_dev_name_entry pci_vendor_list[] = {
	{0x1057, 0x12, "MPC8548E PCI Controller"},
	{0x14e4, 0xb214, "BCM Network Controller"},
	{USB_OHCI_VEND_ID, USB_OHCI_DEV_ID, "OHCI Network Controller"},
	{USB_OHCI_VEND_ID, USB_EHCI_DEV_ID, "EHCI Network Controller"},
	{USB_OHCI_VEND_ID_UPD720102, USB_OHCI_DEV_ID_UPD720102,
                                     "OHCI Network Controller"},
	{USB_OHCI_VEND_ID_UPD720102, USB_EHCI_DEV_ID_UPD720102,
                                     "EHCI Network Controller"},
	{0, 0, ""},
};

unsigned int usb_ohci_vend_id;
unsigned int usb_ohci_dev_id;

static char *
get_pcie_devname (unsigned int vendor, unsigned int device)
{
...
#ifdef CONFIG_RMI
		rmi_dev = PCI_DEV(dev);
		if((bus == 0) && (rmi_link_state[rmi_dev] == 0)) /* PCIE link is !up */
			continue;
#endif
...
#ifdef CONFIG_RMI
		if(rmi_change_class && (bus==0) && !PCI_FUNC(dev)) {
			pci_hose_read_config_dword(hose, dev, 
					PCI_REVISION_ID, &ccode);
			ccode = ccode | 0x06040000;
			...
			printf ("Found %s, (%d, %d, %d), (%x, %x)\n",
					get_pcie_devname(vendor, device), PCI_BUS(dev), PCI_DEV(dev),
					PCI_FUNC(dev), vendor, device);

                        if ((vendor == USB_OHCI_VEND_ID && 
                                        device == USB_OHCI_DEV_ID) || 
                            (vendor == USB_OHCI_VEND_ID_UPD720102 && 
                                        device == USB_OHCI_DEV_ID_UPD720102)) {
			    usb_ohci_vend_id = vendor;
			    usb_ohci_dev_id = device;
			}
		}
	}
