...
spd_sdram(void)
{
	...
	*parkes_sc_pad = 1;
	if (i2c_read(SPD_EEPROM_ADDRESS, 0, 1,
				(uchar *) &spd, sizeof(spd)) == -1) {
		printf("SPD_EEPROM_ACCESS: I2c read to eeprom failed\n");
		return 0;
	}
	printf("Manufacturer part number of DDR %s\n", spd.mpart);
	 ...
	 * has fixed DDR II IO voltage biasing.If 2.0 apply the voltage settings.
	 */
	svr = get_svr();
	major = SVR_MAJ(svr);
	minor = SVR_MIN(svr);
    ...
    if (major == 2 && minor != 0) {
		gur->ddrcdr |=  0x80000;
		printf(" DDRCDR  - DDR CONTROL DRIVER REGISTER = 0x%08x\n",
					                gur->ddrcdr);
	}	
	...
	if (spd.mem_type == SPD_MEMTYPE_DDR) {
		...
		/* per hw team request, change to 6 from 7 */
		clk_adjust = 0x6;
	}
...
setup_laws_and_tlbs(unsigned int memsize)
{
	...
	while (ram_tlb_address < (memsize * 1024 * 1024)
	   ...
	   /* 16M kernel space */
       /* making use of remaing TLB entries for mapping 16M kerenl space 
        * if it is a 1GB RAM
        */
    if (memsize == 1024) {
        mtspr(MAS0, TLB1_MAS0(1, ram_tlb_index, 0));
        mtspr(MAS1, TLB1_MAS1(1, 1, 0, 0, BOOKE_PAGESZ_16M));
        mtspr(MAS2, TLB1_MAS2(E500_TLB_EPN(0xC0000000),
                          0, 0, 0, 0, 0, 0, 0, 0));
        mtspr(MAS3, TLB1_MAS3(E500_TLB_RPN(0x0),
                          0, 0, 0, 0, 0, 1, 0, 1, 0, 1));
        asm volatile("isync;msync;tlbwe;isync");
    }   
