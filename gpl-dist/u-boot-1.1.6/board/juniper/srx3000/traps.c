...
boot_test() 
{
#define MAX_BOOT_TEST 11
#define BOOT_MSG_LEN  100
#undef DEBUG
#ifdef DEBUG
...
#define DEBUGF(x...)
#endif /* DEBUG */
	volatile u_int32_t  *wdt_reg_dis = (u_int32_t *)(CFG_PARKES_BASE + 0x18);
	int i = 0;
	struct boot_test_name_item {
		u_int32_t offset;
		char description[BOOT_MSG_LEN];
	};
	static  struct boot_test_name_item boot_test_name_tbl[] = {
		{ 1, "Reading Flash Size"},
		{ 2, "PCI Init"},
		{ 3, "PCIE Init"},
		{ 4, "STDIO DEVICES"},
		{ 5, "I2CS CPLD Access"},
		{ 6, "I2C ctrl-2 Access to PCA9577_DEV_ADDR for configuring HA port"},
		{ 7, "Sata Ctrl Init"},
		{ 8, "eTsec2 initialize the phy using MGMT_I2C setting through I2C ctrl-2 "},
		{ 9, "Tsec Ctrl Init"},
		{ 10, "POST Test"},
		{ 11, "USB Init"}
	};
	if (gd->memory_addr >= 0x100000) {
		printf("FAILED: Memory address at 0x%08x\n", gd->memory_addr);
		*wdt_reg_dis = 0x00;
		for (; ;) {
			gd->boot_up_flag = 15;
			main_loop ();
	...
	for (i = 0; i < MAX_BOOT_TEST; i++) {
		if (gd->boot_up_flag > boot_test_name_tbl[i].offset) {
			DEBUGF("\n\nPASS: %d - %s\n",
