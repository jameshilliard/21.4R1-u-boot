...
static int tsec_miiphy_read(char *devname, unsigned char addr,
				unsigned char reg, unsigned short *value);
extern void mgmt_i2cphy_setup(void);
...
int tsec_initialize(bd_t * bis, int index, char *devname)
{
	...
	if (!strcmp(dev->name,"eTSEC2")) {
		/* Try to initialize PHY here, and return */
		mgmt_i2cphy_setup();
	}
#endif

	/* eTSEC1 does not support phy */
	if (!strcmp(dev->name,"eTSEC1")) {
		debug("No phy supported for eTSEC1\n");
		return 1;
	} else {
		/* Try to initialize PHY here, and return */
		return init_phy(dev);
...
uint mii_parse_BCM5482_psr(uint mii_reg, struct tsec_private *priv)
{
	uint reg;

	if (priv->link) {
		write_phy_reg(priv, MIIM_BCM5482_EXP_SEL,  MIIM_BCM5482_EXPSEL_OPSTS);
	reg = read_phy_reg(priv, MIIM_BCM5482_EXP_DATA);
	printf("mii_parse_BCM5482_psr = 0x%x\n", reg);
	if (reg & EXP_OPSTS_SERDES_LINK_UP)
	{
		if (reg & EXP_OPSTS_SERDES_SPEED_1000)
			priv->speed = 1000;
		else if (reg & EXP_OPSTS_SERDES_SPEED_100)
			priv->speed = 100;
		else
			priv->speed = 10;
		if (reg & EXP_OPSTS_SERDES_DPLX_FULL)
			priv->duplexity = 1;
		else
			priv->duplexity = 0;
		return 0;
...
static struct phy_info phy_info_BCM5482 = {
	0x00143bcb,
	"Broadcom BCM5482",
	4,
	(struct phy_cmd[]){	/* config */
	...
	(struct phy_cmd[]){	/* startup */
			   ...
			   {MIIM_STATUS, miim_read, &mii_parse_sr},
			   /* Read the Status */
			   {MIIM_BCM5482_PHY_STATUS, miim_read,
				&mii_parse_BCM5482_psr},
			   {miim_end,}
			   },
	(struct phy_cmd[]){	/* shutdown */
			   {miim_end,}
