...
#if defined (CFG_APPL_PARTITIONS_NAME) && defined (MV_PRESTERA_SWITCH)
		printf("\n");
...
void sflash_print_info (MV_SFLASH_INFO * pInfo)
{
...
#if defined(EX2200)
    uint8_t reg = 0;
    extern MV_STATUS mvStatusRegGet(MV_SFLASH_INFO * pFlinfo, MV_U8 * pStatReg);
#endif
    ...
    defined(EX3300)
            case MV_WP_LOW_1OF64:
                printf("\nWrite Protection    : Lower 1 of 64");
                break;
            case MV_WP_LOW_1OF32:
                printf("\nWrite Protection    : Lower 1 of 32");
                break;
            case MV_WP_LOW_1OF16:
                printf("\nWrite Protection    : Lower 1 of 16");
                break;
            case MV_WP_LOW_1OF8:
                printf("\nWrite Protection    : Lower 1 of 8");
                break;
            case MV_WP_LOW_1OF4:
                printf("\nWrite Protection    : Lower 1 of 4");
                break;
            case MV_WP_LOW_1OF2:
                printf("\nWrite Protection    : Lower 1 of 2");
                break;
#endif
            case MV_WP_ALL:
...
#if defined(EX2200)
        if (mvStatusRegGet(pInfo, &reg) == MV_OK)
            printf("    RDSR (%02x)", reg);
#endif
    }
...
int sflash_erase (MV_SFLASH_INFO *pInfo, MV_U32 s_first, MV_U32 s_last)
{
    ...
    defined(EX3300)
extern MV_STATUS mvStatusRegGet (MV_SFLASH_INFO * pFlinfo, MV_U8 * pStatReg);
extern MV_STATUS mvStatusRegSet (MV_SFLASH_INFO * pFlinfo, MV_U8 sr);
MV_STATUS 
ex_mvStatusRegGet (MV_U8 * pStatReg)
{
    MV_U8 value = 0;
    MV_STATUS result;
    
    result = mvStatusRegGet(&sflash, &value);
    if (result == MV_OK)
        *pStatReg = value;
...
MV_STATUS
ex_mvStatusRegSet (MV_U8 sr)
{
    MV_STATUS result;
    
    result = mvStatusRegSet(&sflash, sr);
    return result;
}
...
int sflash_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    else if (strcmp(argv[1], "RDSR") == 0)
    {
        ...
        if (argc == 2) {
            ex_mvStatusRegGet(&value);
            printf("RDSR = %02x\n", value);
            return 1;
        }
        else if (argc == 3) {
            value = simple_strtoul (argv[2], NULL, 16);
            ex_mvStatusRegSet(value);
            return 1;
        }
    }
#endif
