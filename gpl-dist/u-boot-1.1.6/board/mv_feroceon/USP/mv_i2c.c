...
#if !defined(CONFIG_TWSI_SINGLE_MASTER)    
 ...
 */
int
i2c_read_norsta(MV_U8 chanNum, MV_U8 dev_addr, unsigned int offset, int alen, MV_U8* data, int len)
{
	MV_TWSI_SLAVE	twsiSlave;
	...
	if(alen != 0){
 	...
 	if (mvTwsiWrite (chanNum, &twsiSlave, data, 0) != MV_OK)
		return MV_FAIL;
