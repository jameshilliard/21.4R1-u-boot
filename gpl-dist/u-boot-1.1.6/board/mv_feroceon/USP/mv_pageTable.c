...
static unsigned int createPageTable(void)
{
	...
	/* first region 0 MB - 0x20000000(512MB) cacheable/bufferable */
	...
	/* second region 0x20000000 (512MB) - 0x80000000 (2GB) cacheable/bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT|cacheMode, ALL_ACCESS);
	for (; i < 2048 ; i++) 
	{
		*p++ = (entry | (i << 20));
...
#elif defined(CONFIG_EX3300)
	/* first region 0 MB - 0x40000000(1G) cacheable/bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT|cacheMode, ALL_ACCESS);
	for (i = 0; i < 1024; i++) 
	{
		*p++ = (entry | (i << 20));
	...
	/* 2nd region 0x40000000(1G) - 0x80000000(2GB) cacheable/bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT|cacheMode, ALL_ACCESS);
	for (; i < 2048 ; i++) 
	{
		*p++ = (entry | (i << 20));
	...
	/* 3rd region 0x80000000(2GB) - 0x90000000 cacheable/bufferable to 0 */
	entry = L1Entry(SECTION, 0, 0, U_BIT|cacheMode, ALL_ACCESS);
	for (; i < 2304; i++) 
	{
		*p++ = (entry | ((i - 2048) << 20));
	...
	/* 6th region 0xb0000000  - 0xc0000000 cacheable/bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT|cacheMode, ALL_ACCESS);
	for (; i < 3072; i++) 
	{
		*p++ = (entry | (i << 20));
	...
	/* 7th region 0xc0000000  - 0xd0000000 bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT, ALL_ACCESS);
	for (; i < 3328; i++) 
	{
		*p++ = (entry | (i << 20));
	...
	/* 8th region 0xd0000000  - 0xe0000000 cacheable/bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT|cacheMode, ALL_ACCESS);
	for (; i < 3584; i++) 
	{
		*p++ = (entry | (i << 20));
	...
	/* 9th region 0xe0000000-0xf0000000 cacheable/bufferable to 0xf0000000*/
	entry = L1Entry(SECTION, 0, 0, U_BIT|cacheMode, ALL_ACCESS);
	for (; i < 3840; i++) 
	{
		*p++ = (entry | ((i+256) << 20));
	...
	/* 10th region 0xf0000000  - 0xffffffff none cacheable/bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT, ALL_ACCESS);
	for (; i < 4096; i++) 
	{
		*p++ = (entry | (i << 20));
