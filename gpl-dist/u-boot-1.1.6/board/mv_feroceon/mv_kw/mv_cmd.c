...
#if defined(CFG_NAND_BOOT) || defined(MV_INCLUDE_NAND)
#include <nand.h>
...
#if !defined(CONFIG_EX2200) && !defined(CONFIG_RPS200)
int nor_do_mtdburn(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  ...
  uint kernel_addr = 0x2800000;
  uint rootfs_addr = 0x3400000;
  uint total_in;
  int rc;
  int single_file = 0;
  MV_U32 s_first, s_end, s_last;
  int bz2_file = 0;

  addr = load_addr = 0x2000000;

  s_last = flash_in_which_sec(&flash_info[BOOT_FLASH_INDEX], 
                              CFG_APPL_PART_UBOOT_START + CFG_APPL_FLASH_SIZE - 1);
  ...
  s_first = flash_in_which_sec(&flash_info[BOOT_FLASH_INDEX], 
                               CFG_APPL_PART_KERNEL_START);

  s_end = flash_in_which_sec(&flash_info[BOOT_FLASH_INDEX], 
                             CFG_APPL_PART_KERNEL_START + kernel_unc_len -1);

  printf("Un-protect kernel/vxWorks image sectors\n");
  flash_protect (FLAG_PROTECT_CLEAR,
                 CFG_APPL_PART_KERNEL_START,
                 CFG_APPL_PART_KERNEL_START + 
                 (CFG_APPL_PART_KERNEL_SIZE + CFG_APPL_PART_ROOTFS_SIZE) - 1,
                 &flash_info[BOOT_FLASH_INDEX]);
  ...
  flash_write((uchar *)kernel_addr,
              (ulong)CFG_APPL_PART_KERNEL_START,
              (ulong)(kernel_unc_len));

  printf("Done\nProtect kernel/vxWorks image sectors\n");
  flash_protect (FLAG_PROTECT_SET,
                 CFG_APPL_PART_KERNEL_START,
                 CFG_APPL_PART_KERNEL_START + 
                 (CFG_APPL_PART_KERNEL_SIZE + CFG_APPL_PART_ROOTFS_SIZE) - 1,
  ...
  if (!single_file)
  {
    ...
    s_end = flash_in_which_sec(&flash_info[BOOT_FLASH_INDEX],                                CFG_APPL_PART_ROOTFS_START + rootfs_unc_len -1);
    
    printf("Un-protect rootfs sectors\n");
    flash_protect (FLAG_PROTECT_CLEAR,
                   CFG_APPL_PART_ROOTFS_START,
                   CFG_APPL_PART_ROOTFS_START +  CFG_APPL_PART_ROOTFS_SIZE - 1,
                   &flash_info[BOOT_FLASH_INDEX]);
    ...
    flash_write((uchar *)rootfs_addr,
                (ulong)CFG_APPL_PART_ROOTFS_START,
                (ulong)(rootfs_unc_len));
    
    printf("done\nProtect rootfs sectors\n");
    flash_protect (FLAG_PROTECT_SET,
                   CFG_APPL_PART_ROOTFS_START,
                   CFG_APPL_PART_ROOTFS_START +  CFG_APPL_PART_ROOTFS_SIZE - 1,
                   &flash_info[BOOT_FLASH_INDEX]);
  ...
  printf("\nErasing the rest\n");
  printf("Un-protect the rest\n");
  flash_protect (FLAG_PROTECT_CLEAR,
                 CFG_APPL_PART_KERNEL_START,
                 CFG_APPL_PART_KERNEL_START + 
                 (CFG_APPL_PART_KERNEL_SIZE + CFG_APPL_PART_ROOTFS_SIZE) - 1,
                 &flash_info[BOOT_FLASH_INDEX]);
  printf("\nErase %d - %d sectors... ", s_end+1, s_last);

  flash_erase(&flash_info[BOOT_FLASH_INDEX], s_end+1, s_last);

  printf("Protect the rest\n");
  flash_protect (FLAG_PROTECT_SET,
                 CFG_APPL_PART_KERNEL_START,
                 CFG_APPL_PART_KERNEL_START + 
                 (CFG_APPL_PART_KERNEL_SIZE + CFG_APPL_PART_ROOTFS_SIZE) - 1,
...
#endif /* !CONFIG_EX2200 && !CONFIG_RPS200 */
#endif /* defined(CFG_NAND_BOOT) */
