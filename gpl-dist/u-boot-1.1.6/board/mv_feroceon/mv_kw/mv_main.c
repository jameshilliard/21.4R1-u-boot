...
void print_dev_id(void){
	...
	mvBoardNameGet(boardName);
        ...
        printf(" ** MARVELL BOARD: %s BE (%s)\n",
                                 boardName, (db_98dx4122_detected)?"detected and changed from RD-98DX3121-24G" : "configured");
        printf(" ** Linux   - BE support\n");
        printf(" ** vxWorks - BE support\n");
#else
        printf(" ** MARVELL BOARD: %s LE (%s)\n",
                                 boardName, (db_98dx4122_detected)?"detected and changed from RD-98DX3121-24G" : "configured");
...
ethaddr=$(ethaddr) $(netbsd_netconfig)");
  ...
  if(!env)
        {
                char buf[512];
                sprintf(buf, "mtdparts=spi_flash:0x%08x(uboot)ro," 
  ...
  if(!env)
    setenv("image_name","uImage_xcat"); 
        ...
        env = getenv("vx");
        if (!env)
                setenv ("vx", "setenv bootargs mgi(0,0) host:VxWorks h=$(serverip) e=$(ipaddr):FFFF0000 u=anonymous pw=target; setenv bootaddr 0x700; setenv loadaddr 0x1700000; tftpboot $(loadaddr) $(vx_image_name); bootvx "); 
...
int mv_set_power_scheme(void)
{
	...
	/* Close egiga 1 */
        s = getenv("forceGbe1");
	if ( (s[0] != 'y') || ( s[0] != 'Y') || (mppGroupType1 != MV_BOARD_GMII) && (mppGroupType1 != MV_BOARD_RGMII) && (mppGroupType2 != MV_BOARD_RGMII))
	{
