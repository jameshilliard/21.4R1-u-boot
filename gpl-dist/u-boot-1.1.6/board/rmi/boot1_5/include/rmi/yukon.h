...
enum {
	SKY2_STAT_CTRL	= 0x0e80,/* 32 bit	Status BMU Control Reg */
...
struct rx_ring_info {
	...
	uint32_t	data_addr;
	uint32_t 	data_size;
	uint32_t	frag_addr[ETH_JUMBO_MTU >> PAGE_SHIFT];
};
...
struct sky2_priv_data {
	int devno;
	int init_done;
	struct sky2_port * driver_data;
} sky2_priv_data_t;
