...
int do_read64 ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    printf("attempting to read from addr: 0x08%x%08x\n", (uint32_t)((addr>>32)&0xffffffff), (uint32_t)(addr&0xffffffff));
...
int do_read64b ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    printf("attempting to read from addr: 0x%qx\n", addr);
...
int do_write64 ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    printf("writing 0x%qx to addr: 0x%qx\n", val, addr);
...
int do_write64b ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    printf("writing 0x%x to addr: 0x%qx\n", val, addr);
...
int do_mem_cp ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	*/
	...
	while (count-- > 0) {
		if ((count % 1024) == 0) {
		    /*
		     * tickle watchdog while coping loader
		     * image
		     */ 
	...
	/*
	 * watchdog was tickled before copying last 
	 * chunk of data, give a tickle here
	 */ 
	RELOAD_WATCHDOG(PAT_WATCHDOG);
	return 0;
}
...
#ifdef CFG_STATIC_MEMTEST_PATTERN
	ulong	length, i;
#endif
...
#ifdef CFG_STATIC_MEMTEST_PATTERN
	if (argc > 4) {
		if (!strcmp (argv[4], "static")) {
		    printf ("Testing %08x ... %08x: %08x\n", (uint)start, (uint)end, (uint)pattern);
		    length = end - start + 1;
		    for (;;) {
