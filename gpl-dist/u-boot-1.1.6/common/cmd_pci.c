...
static void pci_header_show_hierachy(pci_dev_t dev);
/*
 * Subroutine:  pciinfo
 *
...
/*
 * Subroutine:  pciinfo_all
 *
 * Description: Another version of pciinfo, which show all devices on
 *				all buses, and the bus hierachy.
 *
 * Return:      None
 *
 */
void pciinfo_all()
{
	...
	unsigned char secbus;

	printf("BusDevFun  VendorId   DeviceId   ClassSubC    PriB SecB SubB   ClassName\n");
	printf("________________________________________________________________________________\n");


	BusNum = 0;
	/* Loop on the bus number from 0 till last_busno.  last_busno is updated 
	 * whenever a bridge device is found and the new secondary bus is assigned.
	 */
    while (BusNum <= LastBusNo) {
		for (Device = 0; Device < PCI_MAX_PCI_DEVICES; Device++) {
			HeaderType = 0;
			VendorID = 0;
			for (Function = 0; Function < PCI_MAX_PCI_FUNCTIONS; Function++) {
...
/*
 * Subroutine:  pci_header_show_hierachy
 *
 * Description: A varient of pci_header_show_brief, which 
 *      prints the bus numbers of the bridges.  Used by
 *      the command "pci a" to show the PCI hierachy in the 
 *      system.
 *
 * Inputs:	dev      Bus+Device+Function number
 *
...
static void pci_header_show_hierachy(pci_dev_t dev)
{
	...
	if (class == PCI_BASE_CLASS_BRIDGE) {
		...
		pci_read_config_byte(dev, PCI_SECONDARY_BUS, &secbus);
		pci_read_config_byte(dev, PCI_SUBORDINATE_BUS, &subbus);
	}

	
	printf("0x%.4x     0x%.4x     0x%.2x 0x%.2x    ",
		vendor, device, class, subclass);
	
	if (class == PCI_BASE_CLASS_BRIDGE) {
		printf("0x%.2x 0x%.2x 0x%.2x   ", pribus, secbus, subbus);
	}
	else {
		printf("                 ");
	}
	
	printf("%-8s\n", pci_classes_str(class));
...
int do_pci (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	case 'h':		/* header */
		...
		if ((bdf = get_pci_dev(argv[2])) == -1)
			return 1;
		break;
	case 'r':       /* re-init, i.e., re-scan the pci tree */
		pci_init();
		break;
	case 'a':
		pciinfo_all();  /* show all devices and bridges */
		break;
	default:		/* scan bus */
