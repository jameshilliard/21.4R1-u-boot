...
extern int usb_root_hub_devnum[];
#endif

#ifdef CONFIG_JSRXNLE

extern int usb_nand_format (int device);
extern int nand_phys_page_read(uint32_t pgno, uint8_t *buf);
#define SIGNATURE_COUNT 4

static const uint8_t nand_patch_string[] = {
	0x50, 0x41, 0x54, 0x43, 0x48, 0x20, 0x46, 0x4F,
	0x52, 0x20, 0x50, 0x46, 0x44, 0x20, 0x48, 0x53,
	0x20, 0x56, 0x32, 0x2E, 0x31, 0x2E, 0x30, 0x20,
...
#ifdef CONFIG_JSRXNLE
/*
 * nand_format command interpreter
 */
int do_nandformat (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	CASE_ALL_SRXLE_NF_MODELS
		break;
	default:
		printf("Nand flash is not supported on this platform\n");
		return 0;
	}

	if (usb_nand_format(0) != 0)
		printf("Nand format failed\n");
	else
		printf("Nand format done\n");
	return 0;
}
 ...
 * nand_format command interpreter
 */
int do_nand_patch_verify (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	DECLARE_GLOBAL_DATA_PTR;
	...
	buf = (uint8_t *)(malloc(512 * sizeof(uint8_t)));

	printf("Checking NAND Patch signature:\n");
	for (i = 0; i < SIGNATURE_COUNT; i++) {
		if (nand_phys_page_read((4*0x10)+i, buf)) {
			for (j = 0; j < 32 ; j++) {
				if ((buf[j]) != (nand_patch_string[j])) {
					free(buf);
					printf("\tPatch Missing, Check \
						Failed\n");
					return 0;
				}
			}
	...
	free(buf);
	printf("\tValid Patch Found, Check Passed \n");
	return 0;
}
...
int do_usb (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
#endif
		if (argc == 3) {
			port_n = simple_strtoul(argv[2], NULL, 16);
			printf("stopping USB port: %d..\n", port_n);
		} else {
			port_n = 0;
			printf("stopping USB..\n");
		}
	...
	if (strncmp(argv[1],"tree",4) == 0) {
		...
		for (i = 0; i < usb_num_root_hub; i++) {
			if (usb_root_hub_devnum[i] != -1) 
				usb_show_tree(usb_get_dev_index(usb_root_hub_devnum[i]-1));
		}
#else
...
#if 0   /* avoid compiler warning about unused variable console_buffer */
	...
	if (strcmp(argv[1],"write") == 0) {
		...
		printf("\n**Warning - USB override the following device.**\n");
		stor_dev=usb_stor_get_dev(usb_stor_curr_dev);
		dev_print(stor_dev);
		printf("Continue? (y/n)");
		readline(" ");
		...
		if (argc==5) {
			...
			unsigned long cnt  = simple_strtoul(argv[4], NULL, 16);
			unsigned long n;
			printf ("\nUSB write: device %d block # %ld, "
		                "count %ld ... ",
					usb_stor_curr_dev, blk, cnt);
			stor_dev=usb_stor_get_dev(usb_stor_curr_dev);
			n = stor_dev->block_write
			        (usb_stor_curr_dev, blk, cnt, (ulong *)addr);
...
#ifdef CONFIG_JSRXNLE
U_BOOT_CMD(
	nand_format,	1,	1,	do_nandformat,
	"nand_format - format nand flash\n",
);

U_BOOT_CMD(
        nand_patch_verify,    1,      1,      do_nand_patch_verify,
