...
extern void crc32_wd_buf(const unsigned char *input, unsigned int ilen,
...
struct hash_algo *find_hash_algo(const char *name)
{
	int i;
...
void show_hash(struct hash_algo *algo, ulong addr, ulong len,
		      u8 *output)
{
	int i;
		 ...
		 int argc, char * const argv[])
{
	...
	/* Horrible code size hack for boards that just want crc32 */
		...
		crc = crc32(0, (const uchar *)addr, len);
