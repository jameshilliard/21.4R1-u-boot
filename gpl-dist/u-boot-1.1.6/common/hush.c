...
/* JUNOS begin ***********************************************************
 *
 * On 12.3 based U-Boot images, it is seen that when "diag run cpu"
 * command is executed, main command processing loop from below
 * function was exiting because of flag variable. Though command runs
 * successfully and returns control to calling function below, flag value
 * checking fails and loop is exited there by entering the while(1) loop.
 * Only way to recover the board is by power cycling.
 * There is no change to CPU diag functions code/calling functions code,
 * So this seems to be compiler optimization issue, and code shuffle is
 * making the loop to exit after issuing the commnad "diag run cpu".
 * By making flag variable as volatile this problem is solved even
 * though this is common code, flag variable has been changed to volatile
 * type so that this problem will not occur.
 *
 * JUNOS end ************************************************************/


int parse_stream_outer(struct in_str *inp, volatile int flag)
{
