...
#if defined(CONFIG_ACX) || defined(CONFIG_ACX_HOSTCC)
	...
	*value = (uint8_t *)fdt_getprop(fit, noffset, FIT_VALUE_PROP, &len);
	if (*value == NULL) {
		...
		*value_len = 0;
	    ...
	    debug("\n%s: got hash length = %d from FIT file", __FUNCTION__, len);
	}
				...
				size_t size, char **err_msgp)
{
	...
	*err_msgp = NULL;
	...
	debug("\n Calculating hash on %04x of size %04x", data, size);
	...
	debug("\n hash calculated on data ");
	for (ignore=0; ignore < value_len; ignore++)
		debug(" %02x", value[ignore]);
...
int fit_image_verify(const void *fit, int image_noffset)
{
	...
	for (noffset = fdt_first_subnode(fit, image_noffset);
		...
		if (!strncmp(name, FIT_HASH_NODENAME,
			...
			debug("\n%s: calling fit_image_check_hash", __FUNCTION__);
			...
			debug("\n%s: calling fit_image_check_sig", __FUNCTION__);
			/* Junos start */
			sig_node_present = 1;
			/* Junos end */
	...
	/* Junos start */
	/* If signature node is not present, ignore all images */
	if (sig_node_present)
		return 1;
	else {
	    char *desc;
		...
		/* 
		 * If it is uboot, loader or iCRT images
		 * and no signature node, then return fail
		 */
		debug("\n desc: %s ", desc);
