...
#if defined(CONFIG_FIT_SIGNATURE) || defined(USE_HOSTCC)
#include <image.h>
#include <rsa.h>
		...
		size_t size, int required_keynode, char **err_msgp)
{
	...
	*err_msgp = NULL;
	...
	debug("\n%s: got signature value of len %d", __FUNCTION__, fit_value_len);

	region.data = data;
	region.size = size;

/* Junos start */
/* call rsa_verify() directly */
	/* if (info.algo->verify(&info, &region, 1, fit_value, fit_value_len)) { */
	if (rsa_verify_with_compiled_in_key(&info, &region, 1, 
		...
		int *no_sigsp)
{
...
#ifdef DEBUG
	/* FDT file can be loaded to following address for verification */
	sig_blob = (void *) 0x1000a000;
#endif
	...
	*no_sigsp = 1;
	...
	if (sig_node < 0) {
	    ...
	    debug("%s: got signature node from FDT blob\n", __func__);
	}
	...
	for (noffset = fdt_first_subnode(sig_blob, sig_node);
		...
		debug("%s: calling fit_image_verify_sig()\n", __func__);
