...
#else
#define USB_PRINTF(fmt,args...)    wait_ms(1)
#endif
...
int dev_usb_index;
int usb_multi_root_hub; /* This is set to 1 if there are more than 1 root hubs */
int usb_num_root_hub;   /* Number of root hubs */
int usb_root_hub_devnum[USB_MAX_ROOT_HUB]; /* Dev num for these root hubs */
...
{
	int result = 0;
#ifdef USB_MULTI_CONTROLLER  /* Remove "unused variable" warning */
	int i;
#endif
...
int usb_new_device(struct usb_device *dev)
{
	...
	dev->maxpacketsize = 3;		/* Default to 64 byte max packet size */
...
int usb_scan_devices(void)
{
	...
	if (usb_multi_root_hub) {
		...
		for (i = 0; i < usb_num_root_hub; i++) {
			...
			usb_root_hub_devnum[i] = dev->devnum;
			dev->is_root_hub = 1;
#if defined(USB_MULTI_CONTROLLER)
			root_hub_index = i;
			dev->controller = root_hub_index;
			usb_reset_rootdev();
#endif
			usb_new_device(dev);
			if (!usb_scan_dev)
				printf("Root Hub %d: %d USB Device(s) found\n",
				       i, dev_index-usb_root_hub_devnum[i]+1);
		}
	} else {
		...
		dev->controller = root_hub_index;
#endif
		result = usb_new_device(dev);
		if (!usb_scan_dev)
			printf("%d USB Device(s) found\n",dev_index);
	}

	dev_usb_index = dev_index;
...
void usb_hub_port_connect_change(struct usb_device *dev, int port)
{
	...
	if ((portstatus & USB_PORT_STAT_SPEED) != USB_PORT_STAT_HIGH_SPEED) {
		return;
	}
#endif
