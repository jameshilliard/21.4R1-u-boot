 ...
 * Copyright (c) 2006-2008, 2010, Juniper Networks, Inc.
 *
 * Adapted for U-Boot:
...
/* Flag to check if ST Nand controller is in sane state */
static int probe = 0;
/* Max bootcmd length permissible */
#define BOOTCMD_LEN 100
static char orig_bootcmd[BOOTCMD_LEN + 1];
...
int usb_max_devs = 0; /* number of highest available usb device */
...
int usb_stor_BBB_comdat(ccb *srb, struct us_data *us)
{
	...
	unsigned int pipe;
	/*
	 * XXX - make cbw static for the 2nd stage loader
	 * to work. This needs looking into...
	 */
	static umass_bbb_cbw_t cbw;
...
int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
	...
	/*
	 * XXX - make csw static for the 2nd stage loader
	 * to work. This needs looking into...
	 */
	static umass_bbb_csw_t csw;
...
#ifdef CONFIG_JSRXNLE
/* Function for formatting ST Nand controller based nand flash */
int usb_nand_format (int device)
{
	int retry;
	ccb *srb = &usb_ccb;
	struct us_data *ss = &usb_stor[device];
	char *argv[4] = { "setenv", "nand_error", "0", NULL };

	/* Setting up the SCSI format command */
	memset(&srb->cmd[0],0,12);
	srb->cmd[0]=0xf1;
	srb->cmd[1]=0x04;
	srb->cmd[2]=0x0;
	...
	printf("Formatting will take some time, please wait.......\n");
	while (usb_test_unit_ready(srb,ss)) {
		wait_ms(10000);
		fprintf(stderr, ". ");
	}
	if (simple_strtoul(getenv("nand_error"), NULL, 16) && strlen(orig_bootcmd)) {
		/* Restoring original bootcmd */
...
/* To read physical page from Nand Flash, rather than logical page */
int nand_phys_page_read(uint32_t pgno, uint8_t *buf)
{
    DECLARE_GLOBAL_DATA_PTR;
    ccb *srb = &usb_ccb;
    struct us_data *ss = &usb_stor[0];
    unsigned short blocks = 1;

    switch (gd->board_desc.board_type)  {
        CASE_ALL_SRXLE_NF_MODELS
            break;
        default:
            printf("Nand flash not supported\n"); 
            return 0;
    }
    ...
    srb->cmd[0] = 0xf1;
    srb->cmd[1] = 0x28;
    srb->cmd[2]=((unsigned char) (pgno>>24))&0xff;
    srb->cmd[3]=((unsigned char) (pgno>>16))&0xff;
    srb->cmd[4]=((unsigned char) (pgno>>8))&0xff;
    srb->cmd[5]=((unsigned char) (pgno))&0xff;
    srb->cmd[6] = 0x00;
    srb->cmd[7]=((unsigned char) (blocks>>8))&0xff;
    srb->cmd[8]=(unsigned char) blocks & 0xff;
    srb->cmdlen = 10;
    srb->datalen = 512;
    srb->pdata = (unsigned char *)buf;

    /* Setting priv flag, indicating its a device related
        proprietary protocol extension command */
    srb->priv = SCSI_EXTENSION;
...
retry_it:
#if !defined(CONFIG_USB_SHOW_NO_PROGRESS)
...
#ifdef CONFIG_JSRXNLE
			/* If read error occurs during probe, return immediately.
			 * This means that ST nand controller is in bad state.
			 */
			if (probe)
				return 0;
#endif
...
retry_it:
		...
		srb->datalen=usb_dev_desc[device].blksz * smallblks;
		srb->pdata=(unsigned char *)buf_addr;
		if(usb_write_10(srb,(struct us_data *)dev->privptr,
		                                start, smallblks)) {
			USB_STOR_PRINTF("Write ERROR\n");
			usb_request_sense
			            (srb,(struct us_data *)dev->privptr);
			if(retry--)
				goto retry_it;
			blkcnt-=blks;
			break;
...
int usb_stor_get_info(struct usb_device *dev,struct us_data *ss,block_dev_desc_t *dev_desc)
{
	...
	CASE_ALL_SRXLE_NF_MODELS
		/* 
		 * A workaround for ST nand controller to verify if everything is ok,
		 * so reading block 0 to see if some read error occurs.ST nand controller
		 * has a quirk that it hangs if some non-recoverable ECC error occurs
		 * in nand flash.At that time, it gives continuous read
		 * error and the box hangs. 
		 */
		if ((dev->descriptor.idVendor == USB_STMICRO_NANDFLASH_VENDOR) && \
		   (dev->descriptor.idProduct == USB_STMICRO_NANDFLASH_PRODUCT)) {
			if (!simple_strtoul(getenv("nand_error"), NULL, 16)) {
				/* Setting the probe flag so that it returns back
				 * immediately in case of read error
				 */
				probe = 1;
				if ( !usb_stor_read(dev_desc->dev, 0, 1, (ulong *) buffer) ) {
					char *argv[4] = { "setenv", "nand_error", "1", NULL };

					_do_setenv (0, 3, argv);
					saveenv();
					printf("Error found in nand flash, resetting the board\n");
					printf("Please follow the instructions once board is reset\n");
					do_reset(0, 0, 0, 0);
					return 0;
				}
			}
			else {
				/* Read Error last time, retry read to check whether the nand_flash recovered */
				probe = 1;
				if ( !usb_stor_read(dev_desc->dev, 0, 1, (ulong *) buffer) ) {
					/* retry failed */
					printf("Read Error for %s device %d\n", dev->mf, dev_desc->dev);

					if (strlen(getenv("bootcmd")) > BOOTCMD_LEN) {
						printf("Bootcmd too long, please set it manually after formatting nand flash\n");
						orig_bootcmd[0] = 0;
					} else {
						strcpy(orig_bootcmd,getenv("bootcmd"));
					}
					setenv("bootcmd", "");

					printf("Nandflash devide may halt due to unexpected reset, please issue power cycle.\n");
					printf("If power cycle can't recover the device, please issue command :\n");
					printf("	<nand_format> at console prompt to format \n",dev_desc->dev);
					probe = 0;

					return 1;
				} else {
					/* retry success, clear the nand_error flag */
					char *argv[4] = { "setenv", "nand_error", "0", NULL };
					_do_setenv (0, 3, argv);
					saveenv();
					printf("Nand flash error recovered, continue boot ...\n");
                }
			}
		}
		/* Resetting the probe flag */
		probe = 0;
		break;
	default:
		break;
...
#ifdef CONFIG_RMI
#define RMI_USB_BOUNCE_BUFFER_OFFSET 0x8b600000
#endif

int
usb_disk_scan(int dev)
{
#if defined(CONFIG_OCTEON) || defined(CONFIG_RMI) 
	int count = 0;
#endif
...
#if defined(CONFIG_OCTEON) || defined(CONFIG_RMI) 
#ifdef CONFIG_OCTEON
	bounce_buffer = (void *)JSRXNLE_USB_BOUNCE_BUFFER_OFFSET;
#endif
#ifdef CONFIG_RMI
	bounce_buffer = (void *)RMI_USB_BOUNCE_BUFFER_OFFSET;
#endif
	count = (dev < 0) ? 0 : dev + 1;  
	...
	bounce_buffer = (void *)(gd->ram_size - (1024*1024)); 
#endif
	if (dev < 0)	/* First call: start with first. */
		dev = (USB_DISK_SCAN_NEXT < 0) ? usb_max_devs - 1 : 0;
	else		/* Subsequent call: continue with next after. */
		dev += USB_DISK_SCAN_NEXT;

	while (dev >= 0 && dev < usb_max_devs) {
...
usb_disk_read(int dev, int lba, int totcnt, char *buf)
{
	...
	while (totcnt > 0) {
		...
		rd = usb_stor_read(dev, lba, cnt, (unsigned long *)iobuf);
		if (rd == 0)
			return (totcnt);
		if (iobuf != buf)
