...
int cpu_init_r(void)
{
	...
	case 0x20000000:
		if (ver == SVR_8548 || ver == SVR_8548_E ||
			ver == SVR_P2010 || ver == SVR_P2010_E ||
			ver == SVR_P2020 || ver == SVR_P2020_E) {
			if (verbose)
	...
	case 0x10000000:
		if (ver == SVR_8548 || ver == SVR_8548_E ||
		     ver == SVR_8533 || ver == SVR_8533_E ||
		     ver == SVR_8544 || ver == SVR_8544_E) {
			if (verbose)
				printf ("L2 cache 256KB:");
		       break;
		}
	case 0x30000000:
		if (ver == SVR_8572_E || ver == SVR_8572) {
			if (verbose)
		    		printf("L2 cache 1MB:");
		}
		break;
	...
	asm("msync;isync");
	l2cache->l2ctl = 0x48000000; /* invalidate */
	cache_ctl = l2cache->l2ctl;
	asm("msync;isync");

	l2cache->l2ctl = 0x88000000; /* enable L2 cache */
	cache_ctl = l2cache->l2ctl;
	asm("msync;isync");
