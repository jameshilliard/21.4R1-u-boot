...
/* ACX-500 SYSPLD LPC registers defines */
#define ACX500_SYSPLD_TRANS_SUCCESS	    0x00
...
static u8 acx_syspld_tpm_chk_ready(void)
{
    u32 time_us = MAX_DELAY_US;

    while (time_us > 0) {

	if (syspld_chk_tpm_busy())
	{
	    udelay(1); /* 1 us */
	    time_us--;
	} else {
...
u8 tpm_read_byte(u8 *ptr)
{
	...
	if (acx_syspld_tpm_chk_ready())
	{
	    /* Write ADDR[0:7] to register */
	    syspld_set_lpc_addr_byte1(addr & 0xff);

	    /* Write ADDR[15:8] to register */
	    syspld_set_lpc_addr_byte2(((addr &0xff00) >> 8));		

	    /* Write bit to start transaction & memory read */
	    syspld_start_lpc_mem_read_trans();

	    value = syspld_get_trans_result();
	    udelay(1);

	    if (value == ACX500_SYSPLD_TRANS_SUCCESS)
	    {   
		/* Read data in register to get data from TPM */
		value = syspld_get_lpc_din();
...
#ifdef TPM_DEBUG
	printf("\n tpm_read_word: Addr: %04x :\t  %04x ", ptr, ret);
#endif

	return ret;
}

static void tpm_write_byte(u8 data, u8 *ptr)
{
	if (acx_syspld_tpm_chk_ready())
	{
	    volatile u8 value;
	    ...
	    syspld_set_lpc_dout(data);

	    /* Start transaction */
	    syspld_start_lpc_mem_write_trans();

	    value = syspld_get_trans_result();

	    if (value == ACX500_SYSPLD_TRANS_SUCCESS) {
#ifdef TPM_DEBUG
		printf("\n tpm_write_byte: Addr: %04x = %02x ", ptr, data);
#endif
	    }
	}
}
...
#ifdef TPM_DEBUG
	printf("\n tpm_write_word: Addr: %04x = %04x ", ptr, value);
#endif
