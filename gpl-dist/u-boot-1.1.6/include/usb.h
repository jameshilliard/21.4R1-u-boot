...
#define USB_MAX_ROOT_HUB                2

#define USB_CNTL_TIMEOUT 100 /* 100ms timeout */

/* ST Micro on board NAND flash on JSRXNLE */
#ifdef CONFIG_JSRXNLE
#define USB_STMICRO_NANDFLASH_VENDOR   0x0483
#define USB_STMICRO_NANDFLASH_PRODUCT  0xFADA
#endif /* CONFIG_JSRXNLE end */

#if defined(CONFIG_EX45XX)
/* This extra 20ms delay is defined in FreeBSD.  
 *  But it seems only needed by EX4500 for Sandisk memory stick. 
 */
#define USB_POWER_UP_EXTRA_DELAY    20
#else
#define USB_POWER_UP_EXTRA_DELAY    0
#endif
...
#ifdef CONFIG_EX2200
	/*
	 * There is a tool chain bug that
	 * is causing memory aligment issue
	 * for EX2200.
	 * PR - 969636 
	 */
	unsigned char  dummy;  
...
struct usb_device {
	...
	int controller;
#endif
	char is_root_hub;               /* This device is a root hub */
};
...
#if defined(USB_MULTI_CONTROLLER)
int usb_lowlevel_init_n(int controller);
int usb_lowlevel_stop_n(int controller, uint32_t port_n);
...
int usb_stop(uint32_t port_n); /* stop the USB Controller */
