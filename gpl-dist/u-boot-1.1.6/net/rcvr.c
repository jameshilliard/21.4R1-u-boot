 ...
 * 	as a notification for a systom recovery process.
 */
...
BeaconSend (void)
{
	...
	strcpy(bconpkt->Payload.ModelNumber, "Unknown Model # for HP PyramidPeak NAS system");
    }
...
RecoveryHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	case RCVR_IMAGE_DWNLD:
		...
		case TFTP_OPCODE_WRQ:		    
		    ...
		    else if (rxPacketNumber == (packetNum-1))
		    {
