...
/* amitv : fixme : todo : move these to appropriate header file
 */
extern uint32_t nvram_get_uboot_sreset_test(void); /* acx.c */
extern void nvram_set_uboot_sreset_test(uint32_t); /* acx.c */
#define SRESET_TEST_IN_PROGRESS		0xA18557
...
int secureboot_nvram_wp_post_test(int flags)
{
	...
	/* restore value when this test started */
	/* amitv : not needed, nvram wp once enabled can't be disabled unless
	 * power cycled
	 */
	val = nvram_get_uboot_sreset_test();
	POST_LOG(POST_LOG_INFO,
...
/*
 * This *manual* test is to check the UART transmit disable
 */
int secureboot_uart_tx_post_test(int flags)
{
	int i;

	SB_POST_LOG("UART TRANSMIT DISABLE", "START");

	POST_LOG(POST_LOG_INFO,
		 "We are going to output A-Z. If you see any of the characters\n"
	...
	SB_POST_LOG("UART TRANSMIT DISABLE", "END");

	return 0;
}

/*
 * This *manual* test is to check the UART receive disable
 */
int secureboot_uart_rx_post_test(int flags)
{
	int i;
	int delay;
	int count;
	...
	SB_POST_LOG("UART RECEIVE DISABLE", "START");
	...
	SB_POST_LOG("UART RECEIVE DISABLE", "END");
...
int secureboot_gcr_post_test(int flags)
{
	...
	if (!syspld_close_glass_safe()) {
		...
		/* return status; amitv - fixme , todo, hack for some time*/
	}
