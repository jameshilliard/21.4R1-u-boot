 ...
 * $Id$
 *
 * post.c -- Diagnostics / Power-on Self Test routines for iCRT
 *
...
void post_output_backlog(void)
{
	...
	for (j = 0; j < post_list_size; j++) {
		if (gd->post_log_word & (post_list[j].testid << 16)) {
			post_log("POST: U-boot %s ", post_list[j].cmd);
		...
		int test_flags, int flags, unsigned int i)
{
    ...
    if (flags & (POST_SLOWTEST | POST_POWERON)) {
		/*
		 * POST test can take a lot of time in SLOWTEST or POWERON mode, thus disable
		 */
