...
int
post_bootmode_get(unsigned int *last_test)
{
...
static void
post_log_mark_start(unsigned long testid)
{
#ifdef CONFIG_LOGBUFFER
...
static void
post_log_mark_succ(unsigned long testid)
{
#ifdef CONFIG_LOGBUFFER
...
static void
post_bootmode_test_on(unsigned int last_test)
{
    unsigned long word = post_word_load();
...
static int
post_run_single(struct post_test *test,
...
post_run(char *name, int flags)
{
	...
	if (i < post_list_size) {
	    return post_run_single(post_list + i, test_flags[i], flags, i);
	} else {
	    return -1;
	}
    }
...
static int
post_info_single(struct post_test *test, int full)
{
 ...
 */
unsigned long
post_time_ms(unsigned long base)
{
#ifdef CONFIG_PPC
