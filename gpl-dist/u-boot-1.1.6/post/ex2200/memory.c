...
#undef  INJECT_DATA_ERRORS
	//#define INJECT_DATA_ERRORS
#undef  INJECT_ADDRESS_ERRORS
	//#define INJECT_ADDRESS_ERRORS
...
static int 
memory_post_test1 (unsigned long start,
							  unsigned long size,
							  unsigned long val)
{
	unsigned long i=0;
...
int 
memory_post_test_diag (int flags)
{
    int ret = 0;
    int memory_diag_flag = 0;
    int boot_flag_post = 0;
    unsigned long memsize = gd->ram_size;

    post_result_mem = 0;

    if (flags & BOOT_POST_FLAG) {
        /* test memory space will be used by loader */
        ret = memory_post_tests (0x400000, 0x40000);
        if (ret) {
            post_result_mem = -1;
            return (-1);
        }
    ...
    if (memory_diag_flag)
        post_log("The size of memory %x\n",memsize);   
    
    if (flags & POST_SLOWTEST) {
        if (memory_diag_flag)
            post_log("In slowtest mode \n");

        ret = memory_post_tests (0x400000, memsize);
        if (ret == -1)
    ...
    if (ret == 0) {
        ...
        post_log("-- The memory test                      FAILED @\n");
    return (ret);
}
