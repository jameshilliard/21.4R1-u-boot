...
int memory_post_test_diag (int flags)
{
    ...
    if (rom_flag == 0) {
        if ((flags & BOOT_POST_FLAG == 0) && (flags & POST_DEBUG))
    ...
    if (flags & POST_SLOWTEST) {
        ...
        if (rom_flag == 0) {
            ...
            if (ret == 0)
                ret = memory_post_tests (memsize, 0x800);
            if (ret == 0)
                ret = memory_post_tests (memsize + 0xff800, 0x800);
        }
    }
    ...
    if (rom_flag == 0) {
        if (ret == 0) {
            if (flags & BOOT_POST_FLAG) {
                if (!mem_tests_flag) {
                    lcd_printf_2("POST RAM pass..");
                    return 0;
                }
                else {
                    lcd_printf_2("POST RAM fail..");
                    post_result_mem = -1;
                    return -1;
                }
            }
