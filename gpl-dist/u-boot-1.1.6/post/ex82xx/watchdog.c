...
int watchdog_post_test(int flags)
{
	...
	if (flags & POST_REBOOT) {
		...
		mtspr(TCR, val);
		val = mfspr(TCR);
		post_log("This cmd tests the functionality of CPU watchdog.\n");
		post_log(" This test will RESET the CPU within 10 seconds and\n");
		post_log(" control will be back to the U-Boot prompt\n\n");
