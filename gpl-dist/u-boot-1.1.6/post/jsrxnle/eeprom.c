...
eeprom_post_test (int flags)
{
    ...
    hi_code = jsrxnle_read_eeprom(offset); 
    
    udelay(INTER_BYTE_DELAY_MS);
    
    offset = jsrx_jedec_codes[jedec_lo_bits].offset;
    lo_code = jsrxnle_read_eeprom(offset);
    
    if ((hi_code != jsrx_jedec_codes[jedec_hi_bits].id ||
         lo_code != jsrx_jedec_codes[jedec_lo_bits].id)) {
        post_log("EEPROM Test: Failed\n");
        SET_POST_RESULT(gd->post_result, EEPROM_POST_RESULT_MASK, POST_FAILED);
        return -1;
    }

    /* set result */
    SET_POST_RESULT(gd->post_result, EEPROM_POST_RESULT_MASK, POST_PASSED);
