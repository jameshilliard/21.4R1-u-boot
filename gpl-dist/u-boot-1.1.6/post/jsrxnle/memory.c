...
static uint32_t
srx210_cache_evict_address (uint32_t src_addr, uint32_t increment)
{
    uint32_t    evict_addr, l2cSet, mid, high;

    l2cSet = ((src_addr >> 14) & 0x1FFUL) ^ ((src_addr >> 7) & 0x1FFUL);

    evict_addr = src_addr + increment;  /* One cache block more */

    mid = (evict_addr >> 7) & 0x1FFUL;
    high = mid ^ l2cSet;
    evict_addr = (evict_addr & 0xFFF01FFF) | (high << 14);

    return evict_addr;
}

static uint32_t
srx240_cache_evict_address (uint32_t src_addr, uint32_t increment)
{
    uint32_t    evict_addr, l2cSet, mid, high;
...
flush_cache_line (unsigned long *src_addr)
{
    ...
    CASE_ALL_SRX220_MODELS
        evict_address = srx210_cache_evict_address((uint32_t)src_addr, 0x80);
        break;

    CASE_ALL_SRX650_MODELS
    CASE_ALL_SRX240_MODELS
	evict_address = srx240_cache_evict_address((uint32_t)src_addr, 0x80);
	break;
    default:
	return;
    }
...
memory_post_tests (unsigned long start, unsigned long size)
{
	...
	if (ret == 0) {
		printf("OK\n");
		printf("Checking 512K memory for U-Boot... ");
		ret = memory_post_test1 (start, size, 0xAAAAAAAA);

		/* we took quite a while to test this memory */
		RELOAD_CPLD_WATCHDOG(PAT_WATCHDOG);		
	}
...
memory_post_test (int flags)
{
    ...
    /* test 1 MB */
    ret = memory_post_tests (CFG_SDRAM_BASE, (512 * 1024));
