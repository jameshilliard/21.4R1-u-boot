...
usb_post_test (int flags)
{
	...
	for(i=0;i<USB_MAX_DEVICE;i++) {
	    ...
	    if(dev==NULL) {
		SET_POST_RESULT(gd->post_result, USB_POST_RESULT_MASK,
                                POST_FAILED);
		return -1;
	    }
    ...
    if(usb_pass_flag) {
        SET_POST_RESULT(gd->post_result, USB_POST_RESULT_MASK, POST_PASSED);
        return 0;
    }
    else {
        post_log("USB Test: Failed\n");
        SET_POST_RESULT(gd->post_result, USB_POST_RESULT_MASK, POST_FAILED);
	return -1;
    }
}
