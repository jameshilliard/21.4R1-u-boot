...
struct tsec_device post_etsec_dev[] =
			{{CONFIG_MPC85XX_TSEC3_NAME, LB_MAC | LB_PHY},
			{NULL , 0}
};
               ...
               int debug_flag, int reg_dump)
{
	...
	if (!boot_flag_post) 
		printf("Send packets of various sizes (%d .. %d) and check" 
			" received packets\n\n", MIN_PACKET_LENGTH, max_pkt_len);
...
static int test_loopback(struct tsec_device* eth_dev)
{
	...
	if (!fail) {
		post_log("\nTSEC test completed, 1 pass, 0 errors\n");
		if (ether_debug_flag) {
			for (i = 0; eth_dev[i].name; i++) {
				for (j = 0; j < MAX_LB_TYPES; j++) {
					if (eth_dev[i].capability & (1 << j))
		...
		post_log("\nTSEC test failed, 0 pass, 1 errors\n");
		if (ether_debug_flag) {
			for (i = 0; eth_dev[i].name; i++) {
				for (j = 0; j < MAX_LB_TYPES; j++) {
					if (eth_dev[i].capability & (1 << j)) {
