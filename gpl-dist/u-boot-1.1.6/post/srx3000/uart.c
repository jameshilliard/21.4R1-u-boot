...
test_ctlr (int ctlr, int index)
{
	...
	if (int_loop_flag == 0) {
		for (time = 0; time < MAX_LOOP_CHAR_CNT; time++) {
			ctlr_proc[ctlr].putc ('.');
			ch = uart_serial_getc_diag();
			if (ch == '.') {
				break;
			}
			/* Wait 70ms before the next character reached */ 
			udelay(70000);
		}
		if (time == MAX_LOOP_CHAR_CNT) {
			return -1;
		}
	}
...
static int
uart_serial_init_internal (void)
{
	int clock_divisor;
#ifdef CFG_NS16550_COM1
...
static void
x_serial_putc (const char c,const int port)
{
	if (c == '\n') {
		NS16550_putc(PORT,'\r');
		return;
...
void
uart_serial_putc (const char c)
{
	x_serial_putc(c,CONFIG_CONS_INDEX);
}

int
x_serial_getc (int port)
{
	return NS16550_getc(PORT);
}

int
x_serial_getc_diag (int port)
{
	return NS16550_getc_diag(PORT);
}

int
uart_serial_getc (void)
{
	return x_serial_getc(CONFIG_CONS_INDEX);
}

int
uart_serial_getc_diag (void)
{
...
uart_post_test (int flags)
{
	...
	if (uart_flag) {
		...
		for (i = 0; i < MAX_CHAR_DISP; i++) {
			post_log(" i = %d, ch1 = %c ,  ch1 = 0x%x\n\n",i, ch1[i],ch1[i]);
		}
	}
	int_loop_flag = 0;
	post_log(" Please disconnect the console cable and insert a "
			"loopback cable\n");
