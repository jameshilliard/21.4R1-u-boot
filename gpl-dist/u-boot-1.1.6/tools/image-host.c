...
static uint8_t sigdata[RSA3072_BYTES];
...
/* Junos start */
static int read_signature_file(const char *keydir, void *keydest,
			       ...
			       int image_noffset)
{
    ...
    FILE *f;
    uint8_t os;
    int loop, manual_sig = 0;
    char *desc;

    /* Get OS name from property data */
    fit_image_get_os(fit, image_noffset, &os);

    /* If desc exist and what we want */
    if (!fit_get_desc(fit, image_noffset, &desc)) {

	if (!(strcmp(desc, "uboot"))) {
	    snprintf(path, sizeof(path), "%s/%s.sig", keydir, (char *) desc);
	    manual_sig = 1;
	} else if (!(strcmp(desc, "loader"))) {
	    snprintf(path, sizeof(path), "%s/%s.sig", keydir, (char *) desc);
	    manual_sig = 1;
	} else if (!(strcmp(desc, "icrt"))) {
	    snprintf(path, sizeof(path), "%s/%s.sig", keydir, (char *) desc);
	    manual_sig = 1;
	} else {
	    printf("\n Required signature file not found");
	    printf("\n mkimage generating signatures");
	    return 0;
	}
	...
	if (manual_sig) {
		...
		printf("\n Reading signature from %s file", path);
	    }

	    ret = fread(sigdata, 1, sizeof(sigdata), f);
	    printf("\n%s Signature file read of length %d ", path, ret);
	    if (ret)
	    {
		printf("\nData:\n");
		for(loop = 0; loop < ret; loop++)
		printf(" %02x", sigdata[loop]);
		printf("\n\n");
		return manual_sig;
	    }
	}
...
/* Junos */
{
	...
	/* Junos */
	if (manual_sig) {
	    debug("\n writing signature read from file:\n");
	    int i;
	    for(i=0; i < value_len; i++) {
		    /* Insert the signature values read from file */
		    value[i] = sigdata[i]++;
		    debug("%lx ", value[i]);
	    }
	}
	...
	if (keydest)
		debug("\n destination blob is not NULL ");
	else
		debug("\n destination blob is NULL, so not writing public key");
		...
		int require_keys)
{
	...
	for (noffset = fdt_first_subnode(fit, image_noffset);
		...
		if (!strncmp(node_name, FIT_HASH_NODENAME,
			...
			manual_sig = read_signature_file(keydir, keydest,
							   fit, image_name,
							   noffset, data, size,
							   comment,
							   require_keys,
			...
			ret = fit_image_process_sig(keydir, keydest,
				...
				comment, require_keys, manual_sig); 
		}
		if (ret)
			return -1;
	}
