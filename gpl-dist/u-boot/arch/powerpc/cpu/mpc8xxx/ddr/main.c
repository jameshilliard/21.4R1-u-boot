...
/* JUNOS Start */
	...
	/*
	 * EEPROM is sitting behind a PCA9548's channel 7.
	 */
	int i2c_ret;
	u8 addr = 0x70; /* PCA9548's address */
	u8  val = 0x80; /* channel 7 */

	i2c_ret = i2c_write(addr, 0x00, 1, &val, 1);
	if (i2c_ret) {
		printf("DDR: failed to write to PCA9548 %u\n", addr);
...
phys_size_t fsl_ddr_sdram(void)
{
	...
	/* JUNOS start */
#ifdef CONFIG_RESTRICT_RAM
	if (total_memory > CONFIG_MAX_MEM_MAPPED)
