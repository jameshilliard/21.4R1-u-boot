...
/* JUNOS Start */
#if defined(CONFIG_UBOOT_UPGRADE) || defined(CONFIG_READONLY)
int check_upgrade(void);
#endif

#if defined(CONFIG_EX45XX) || defined(CONFIG_EX4300)
extern int board_start_usb(void);
extern void board_get_enetaddr(uchar * enet);
#endif

#ifdef CONFIG_CONSOLE_SELECT
extern int console_select(void);
#endif
/* JUNOS End */
...
ulong get_effective_memsize(void)
{
	...
	/* JUNOS Start */
	return ((gd->ram_size > CONFIG_MEM_RELOCATE_TOP) ?
		CONFIG_MEM_RELOCATE_TOP : gd->ram_size);
	/* JUNOS End */
#endif
}
...
void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	/* JUNOS Start */
#if defined(CONFIG_UBOOT_UPGRADE)
	check_upgrade();
#endif
	/* JUNOS End */
 ...
 */
	...
	/* JUNOS Start */
#if defined(CONFIG_EX45XX) || defined(CONFIG_EX4300)
	/* EX4500 mgmt port is eTSEC4 */
	/* EX4510 mgmt port is eTSEC1 */
	/* EX4300 mgmt port is FM1@dTSEC1 */
	...
	/* JUNOS Start */
	/* Call misc_init_r earlier, so that
	 * the the peripherals are out of reset,
	 * which are brought our of reset as a apart of
	 * misc_init_r
	 */
