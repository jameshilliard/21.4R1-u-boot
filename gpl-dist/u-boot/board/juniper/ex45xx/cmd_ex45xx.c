        ...
        case 'R':
            ...
            if ((cmd1 == 'o') || (cmd1 == 'O')) {
                ...
                value = readl(ohci_base_addr | reg);
                    printf("OHCI reg 0x%x (0x%x) = 0x%x\n",
                ...
                printf("EHCI reg 0x%x (0x%x) = 0x%x\n",
...
#ifdef CONFIG_EX4510
tsec_mii_t tsec_mii_1310_table[] = {
    { 0x00, 0x03, "control" },
    { 0x01, 0x03, "status" },
    { 0x02, 0x3f, "PHY ID 1" },
    { 0x03, 0x3f, "PHY ID 2" },
    ...
    { 0x0a, 0x01, "1000-T status" },
    { 0x0f, 0x3f, "ext status" },
    { 0x10, 0x7f, "phy spec control" },
    { 0x11, 0x43, "phy spec status" },
    { 0x12, 0x03, "int enable" },
    { 0x13, 0x03, "int status" },
    { 0x14, 0x7f, "ext phy control" },
    { 0x15, 0x3f, "error" },
    { 0x16, 0x3f, "page" },
    { 0x17, 0x3f, "global status" },
    { 0x18, 0x3f, "led control" },
    { 0x19, 0x3f, "led overwrite" },
    { 0x1a, 0x7f, "ext spec control 2" },
    { 0x1b, 0x3f, "ext spec status" },
    { 0x1c, 0x3f, "VCT skew/swap" },
    { 0x1e, 0x3f, "ext address" }
};

tsec_mii_t tsec_mii_bcm54616s_table[] = {
    { 0x00, 0x01, "control" },
    { 0x01, 0x01, "status" },
    { 0x02, 0x01, "PHY ID 1" },
    { 0x03, 0x01, "PHY ID 2" },
    { 0x04, 0x01, "auto adv base" },
    { 0x05, 0x01, "auto partner ab" },
    { 0x06, 0x01, "auto adv ex" },
    { 0x07, 0x01, "auto next" },
    { 0x08, 0x01, "auto partner next" },
    { 0x09, 0x01, "1000-T control" },
    { 0x0a, 0x01, "1000-T status" },
    { 0x0f, 0x01, "IEEE ext status"},
    { 0x10, 0x01, "phy extnded ctrl"},
    { 0x11, 0x01, " phy extended status"},
    { 0x12, 0x01, "receive error counter"},
    { 0x13, 0x01, "false carrier sense counter"},
    { 0x14, 0x01, "receiver NOT_OK counter"},
    { 0x15, 0x01, "expansion reg"},
    { 0x19, 0x01, "AUX status"},
    { 0x1A, 0x01, "INT STATUS"},
    { 0x1B, 0x01, "INT MASK"},
    { 0x1D, 0x01, "mstr/slv speed"},
    { 0x1E, 0x01, "test reg"}
};

/* Shadow reg values for reg 0x1C */
uint16_t shdw_reg[16] = {0x0800,0x0C00, 0x1000, 0x1400,
                         0x2000, 0x2400, 0x2800, 0x3400,
                         0x3800, 0x3C00, 0x4400, 0x4C00,
                         0x5400, 0x6000, 0x7800, 0x7C00};
uint16_t shdw_reg_18[5] = {0x0007, 0x1007, 0x2007, 0x4007, 0x7007};
#endif
...
tsec_phy_dump (char *dev)
{
    ...
    if ((!strcmp(dev, "eTSEC3")) && (priv1112 && priv1112->phydev)) {
        mdio_mode_set(2, 1);
        ...
        for (i = 0; 
            ...
            for (j = 0; j < 7; j++) {
                if (tsec_mii_1112_table[i].pmap & (1 << j)) {
                    temp = (uint16_t)read_phy_reg_private(priv1112, 0x16);
                    temp = ((temp & 0xff00) | j);
                    write_phy_reg_private(priv1112, 0x16, temp);
                    udelay(10);
                    temp1 = (uint16_t)read_phy_reg_private(priv1112, 
        ...
        for (i = 0; 
            ...
            for (j = 0; j < 6; j++) {
                if (tsec_mii_1145_table[i].pmap & (1 << j)) {
                    temp = (uint16_t)read_phy_reg_private(priv1145, 0x16);
                    temp = ((temp & 0xff00) | j);
                    write_phy_reg_private(priv1145, 0x16, temp);
                    udelay(10);
                    temp1 = (uint16_t)read_phy_reg_private(priv1145, 
...
#ifdef CONFIG_EX4510
    struct tsec_private *priv = NULL;
    unsigned int num_table_elements;

    eth_dev = eth_get_dev_by_name(dev);
    priv = (struct tsec_private *) eth_dev->priv;
    if ((board_id() == I2C_ID_JUNIPER_EX4510_32_F) && (version_major() == 1)) {
	num_table_elements = sizeof(tsec_mii_1310_table);
	num_table_elements /= sizeof(tsec_mii_1310_table[0]);
	printf("%s phy 1310 dump\n",dev);
	if (priv && priv->phydev) {
	    printf("page %30s-0--  -1--  -2--  -3--  -4--  -5--  -6--\n", " ");
	    for (i = 0; i < num_table_elements; i++) {
		printf("Register %2d  %-20s",
		    tsec_mii_1310_table[i].reg,
		    tsec_mii_1310_table[i].name);
		for (j = 0; j < 7; j++) {
		    if (tsec_mii_1310_table[i].pmap & (1 << j)) {
			temp = (uint16_t)read_phy_reg_private(priv, 0x16);
			temp = ((temp & 0xff00) | j);
			write_phy_reg_private(priv, 0x16, temp);
			udelay(10);
			temp1 = (uint16_t)read_phy_reg_private(priv,
			    tsec_mii_1310_table[i].reg);
			printf("  %04x", temp1);
		    } else {
			printf("      ");
	...
	num_table_elements = sizeof(tsec_mii_bcm54616s_table);
	num_table_elements /= sizeof(tsec_mii_bcm54616s_table[0]);
	printf("%s phy 54616s dump\n",dev);
	for (i = 0; i < num_table_elements; i++) {
	    printf("Register %2d  %-20s",
		tsec_mii_bcm54616s_table[i].reg,
		tsec_mii_bcm54616s_table[i].name);
	    temp1 = (uint16_t)read_phy_reg_private(priv,
		tsec_mii_bcm54616s_table[i].reg);
	    printf("  %04x\n", temp1);
	}
	printf("reg 0x1c shadow reg values\n");
	for (i = 0; i < 16; i++) {
	    write_phy_reg_private(priv, 0x1C, shdw_reg[i]);
	    temp1 = (uint16_t)read_phy_reg_private(priv, 0x1C);
	    printf("shadow reg: %x, val = %x\n", shdw_reg[i], temp1);
	}
	printf("reg 0x18 shadow reg values\n");
	for (i = 0; i < 5; i++) {
		write_phy_reg_private(priv, 0x18, shdw_reg_18[i]);
		temp1 = (uint16_t)read_phy_reg_private(priv, 0x18);
		printf("shadow reg: %x, val = %x\n", shdw_reg_18[i], temp1);
	}
...
#ifdef CONFIG_EX4510
void
tsec_phy_write_4510 (char *dev, int page, int reg, int value)
{
	...
	if ((board_id() == I2C_ID_JUNIPER_EX4510_32_F) && (version_major() == 1)) {
	    temp = (uint16_t)read_phy_reg_private(priv, 0x16);
	    write_phy_reg_private(priv, 0x16, page);
	    udelay(10);
	    write_phy_reg_private(priv, reg, value);
	    udelay(10);
	    write_phy_reg_private(priv, 0x16, temp);
	} else {
	    udelay(10);
...
mac_reg_dump (char *dev)
{
   ...
   regs = ((struct tsec_private *)eth_dev->priv)->regs;
   /* Adjust the offset to the start of MAC section */
   regs = (volatile uint32_t *) ((unsigned int)regs + 0x500);
    ...
    switch (cmd1) {                        
        ...
        case 'P':
            if ((cmd2 == 'd') || (cmd2 == 'D')) { /* dump */
				...
				tsec_phy_write_4510(dev, page, reg, value);
			}
#endif
            else
                goto usage;
...
/*
 * On EX4510, flash is in a 8 bit mode,
...
{
    uint8_t present;
    int controller = i2c_get_bus_num();
    uint8_t tdata;
...
m1_80_id (int *rev, uint16_t *id, uint8_t *addr)
{
    ...
    if (m1_80_present()) {
        if (i2c_mux(1, CFG_I2C_C2_9546SW2, 2, 1))
            return (-1);
        if (i2c_read(0x52, 4, 1, (uint8_t *)&mid, 2) == 0) {
            *rev = 1;
            *id = mid;
            *addr = 0x52;
        }
            ...
            i2c_write(0x70, 0, 0, &ch, 1);
            if (i2c_read(0x72, 4, 1, (uint8_t *)&mid, 2) == 0) {
                *rev = 2;
                *id = mid;
                *addr = 0x72;
            }
 ...
 */
int 
do_i2c_ex45xx (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    switch (cmd1) {
        ...
        case 'F':
            ...
            if ((argc == 3) && ((argv[1][1] == 'd') ||(argv[1][1] == 'D'))) {
                ...
                i2c_fdr(fdr, CONFIG_SYS_I2C_SLAVE);
                return (1);
            }
...
#ifdef CONFIG_EX4510
 { "EX4510-32T"     , I2C_ID_JUNIPER_EX4510_32_T, "N/A       " ,0x40},
 { "EX4510-32F"     , I2C_ID_JUNIPER_EX4510_32_F, "N/A       " ,0x40},
#endif
 {  NULL             , 0                          }
};
...
do_mfgcfg (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    if (argc >= 3) 
    {
	    ...
	    if (!strcmp(argv[1], "system")) {
		printf("\n HW Re-Work Revsion: '%.2s'\n"   ,tdata + 0x31);
		nbytes = readline("HW Re-Work Revsion: ");
		if (nbytes)
		    memcpy(tdata + 0x31, console_buffer,2);
	    }
...
do_si(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    NS16550_t s_port = (NS16550_t)CONFIG_SYS_NS16550_COM1;
    unsigned char ch;
    uint8_t tdata[20], fm;
    char *arg = "0";
...
U_BOOT_CMD(
    ...
    "    - dump phy registers\n"
#ifdef CONFIG_EX4510
	"tsec phy write <page> <reg> <value>\n"
	"   - write phy registers\n"
	"tsec send pkts\n"
	"   - send pkts on tsec i/f\n"
#endif
    "tsec cs read  <reg>\n"
