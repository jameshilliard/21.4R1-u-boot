...
static char board_name [3][LCD_LINE_LENGTH+1] =
{
	"EX4500-40F      ",
	"EX4510-32T      ",
	"EX4510-32F      "
...
lcd_write (LcdDataType dtype, uint8_t data)
{
	if (lcd_wait_till_not_busy(dtype ? EPLD_LCD_DATA_WRITE : EPLD_LCD_INST_WRITE)) {
		...
		/* Delay of 100 micro seconds to resolve LCD char corruption */
		udelay(100);        
...
lcd_init (int unit)
{
	...
	case I2C_ID_JUNIPER_EX4510_32_F:
	    ...
	    break;
	default:
	    puts("unknown baord type, lcd display will be incorrect\n");
	    break;
	}
	memcpy( lcd_data[0], board_name[board_index], LCD_LINE_LENGTH-1);
#endif
