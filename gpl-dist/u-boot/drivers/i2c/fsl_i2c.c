...
/* JUNOS start */
int i2c_read_noaddr(u8 dev, u8 *data, int length)
{
	int ret = 0;
	...
	if (ret == 0) {
	    if (length) { 
		if (i2c_write_addr(dev, I2C_READ_BIT, 0) == 0)
		    ret = -1; 
	    }
	    if (ret == 0) {
...
int i2c_write_noaddr(u8 dev, u8 *data, int length)
{
	int i = -1;
	int ret = 0;
...
int i2c_read_norsta(u8 dev, uint addr, int alen, u8 *data, int length)
{
	int ret = 0;
	u8 *a = (u8*)&addr;
