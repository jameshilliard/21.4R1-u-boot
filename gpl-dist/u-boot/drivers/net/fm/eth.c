...
static int fm_eth_recv(struct eth_device *dev)
{
	...
	while (!(status & RxBD_EMPTY)) {
		if (!(status & RxBD_ERROR)) {
			...
			/* Skip 4 bytes of appended CRC from data buffer */
			len = rxbd->len - 4;
			NetReceive(data, len);
		} else {
			printf("%s: Rx error\n", dev->name);
...
static int init_phy(struct eth_device *dev)
{
	...
	/* JUNOS Start */
	/* EX4300 uses only one dTSEC interface (dTSEC1). So, check only
	 * for dTSEC1 connection failure and report error for dTSEC1 only.
	 * Ignore errors for other dTSEC interfaces for EX4300 platform.
	 */
	if (!phydev) {
#ifdef CONFIG_EX4300
		if (!strncmp(dev->name, "FM1@DTSEC1", strlen(dev->name)))
