...
#ifdef not_JUNOS	/* JUNOS */
	/* Upload the Fman microcode if it's present */
	rc = fman_upload_firmware(index, &reg->fm_imem, addr);
#else		/* JUNOS Start */
	/* 
	 * Read the FMan microcode from an array (in fm_ucode.h)
	 * and upload.
	 */
	rc = dpa_fm_firmware_upload();
#endif		/* JUNOS End */
	if (rc)
		return rc;
...
/*
 * Perform the task of actual uploading of FMAN microcode to FMAN memory.
 * Follow the steps mentioned in the DPA RM (section 8.10.3.4) for
 * uploading microcode to FMAN memory.
 */
static int
dpa_fm_ucode_upload(u_int32_t *ucode, u_int32_t size)
{
	fm_ctrl_cfg_data_t *fm_cfg = (fm_ctrl_cfg_data_t *)ADDR_FMAN_CTRL_CFG;
	uint32_t i;
	uint32_t mod16;
	uint32_t tmpval;

	/* 1. Initialize FMCDADDR ADDR field to zero and enable auto increment */
	fm_cfg->fm_cdaddr = FMC_ADDR_AUTO_INCR;

	/* 2. Place the barrier to ensure that ADDR is updated before proceeding */
	barrier();

	/* 3. Loop over all of the configuration data and write it to fman memory */
	for (i = 0; i < size / 4; i++) {
		fm_cfg->fm_cddata = ucode[i];
		barrier();
	}

	/*
	 * 4. Continue filling the memory until we have a multiple of 16 bytes
	 *    (this is done to avoid ECC errors).
	 */
	mod16 = (size % 16);
	if (mod16) {
		for (i = 0; i < ((16 - mod16) / 4); i++) {
			/* Write 0xffffffff to FMCDDATA to pad configuration data */
			fm_cfg->fm_cddata = 0xffffffff;
			barrier();
		}
	}

	/* verify each written word to know if the microcode got properly uploaded */
	fm_cfg->fm_cdaddr = FMC_ADDR_AUTO_INCR;
	barrier();
	for (i = 0; i < size / 4; i++) {
		barrier();
		tmpval = fm_cfg->fm_cddata;

		if (tmpval != ucode[i]) {
			printf("Fman: microcode upload error at word %d"
	...
	/*
	 * 5. Set ready in FMCDREADY to allow access of the configuration data to
	 * the FMan controller
	 */
	fm_cfg->fm_cdready = FMC_READY_SET;
	barrier();
...
/*
 * Determine the location of microcode, and upload it to FMAN memory
 */
int
dpa_fm_firmware_upload(void)
{
	uint32_t u_code[] = FM_UCODE_IMAGE;
	uint8_t *ucode_ptr = (uint8_t *)u_code;
	uint32_t u_code_size = sizeof(u_code);

	dpa_fm_ucode_upload(u_code, u_code_size);
	printf("Uploaded FMAN microcode version %u.%u\n",
	ucode_ptr[5], ucode_ptr[7]);
