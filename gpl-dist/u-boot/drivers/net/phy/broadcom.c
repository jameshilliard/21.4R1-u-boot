...
#define MIIM_BCM54XX_AUTODET_MEDIUM		0x7800
#define BCM_SHDW_REG_LED1_SEL_WRITE		0xB4EA   /* LED_1 and 2 speed-multicolor1 */
#define BCM_SHDW_REG_LED2_SEL_WRITE		0xB8E3   /* LED_3 and 4  link/activity */
#define BCM_SHDW_REG_LED_ACT_CNTRL_WRITE	0xA418   /* LED3 - Link+Activity */

/* auto detect medium settings 1c-shadow(11110) */
#define BRGPHY_1C_ADMR_INVERT_FIBER_SD		(1 << 8)   /* invert fiber signal detect pin */
#define BRGPHY_1C_ADMR_FIBER_INUSE_LED		(1 << 7)   /* fiber in use LED */
#define BRGPHY_1C_ADMR_FIBER_LED_MODE		(1 << 6)   /* fiber led mode */
#define BRGPHY_1C_ADMR_FIBER_AUTO_PD		(1 << 4)   /* power down fiber when SD inactive */
#define BRGPHY_1C_ADMR_MEDIUM_DEFAULT		(1 << 2)   /* auto-detect default medium */
#define BRGPHY_1C_ADMR_MEDIUM_PRIORITY		(1 << 1)   /* auto-detect medium priority */
#define BRGPHY_1C_ADMR_AUTODET_MEDIUM_EN	(1 << 0)   /* enable auto detect medium */

/* JUNOS Start */
static u32 bcm54616s_sel_copper_regs(struct phy_device *phydev)
{
	unsigned int mii_reg;

	mii_reg = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD);
	mii_reg &= ~(BRGPHY_1C_MCR_1000BASEX_REG_EN);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD,
			mii_reg | MIIM_BCM54XX_SHD_WRITE);
	return (0);
}

static u32 bcm54616s_sel_fiber_regs(struct phy_device *phydev)
{
	unsigned int mii_reg;
...
static u32 bcm54616s_auto_det_config(struct phy_device *phydev)
{
	unsigned int mii_reg;

	mii_reg = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD);
	mii_reg |= (BRGPHY_1C_ADMR_INVERT_FIBER_SD |
		  BRGPHY_1C_ADMR_FIBER_LED_MODE |
		  BRGPHY_1C_ADMR_MEDIUM_DEFAULT |
		  BRGPHY_1C_ADMR_MEDIUM_PRIORITY | \
		  BRGPHY_1C_ADMR_AUTODET_MEDIUM_EN);
...
/* Broadcom BCM54616S */
static int bcm54616s_config(struct phy_device *phydev)
{
	unsigned int reg;
	...
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_GBIT_CONTROL,
		MIIM_GBIT_CONTROL_INIT);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_ANAR, MIIM_ANAR_INIT);
#if defined(CONFIG_EX62XX)
	/* select copper mode registers */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD, MIIM_BCM54XX_MODE_CTRL);
	bcm54616s_sel_copper_regs(phydev);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_CONTROL, MIIM_CONTROL_INIT);

	/* select fiber registers */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD, MIIM_BCM54XX_MODE_CTRL);
	bcm54616s_sel_fiber_regs(phydev);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_CONTROL, (MIIM_CONTROL_INIT & ~(MIIM_ANEN)));

	/* Enable copper/fiber auto detection */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD, MIIM_BCM54XX_AUTODET_MEDIUM);
	bcm54616s_auto_det_config(phydev);
#endif
#if defined (CONFIG_EX4510) || defined(CONFIG_EX4300)
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_CONTROL, MIIM_CONTROL_INIT);
#endif	
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD, BCM_SHDW_REG_LED1_SEL_WRITE);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD, BCM_SHDW_REG_LED2_SEL_WRITE);
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_BCM54XX_SHD, BCM_SHDW_REG_LED_ACT_CNTRL_WRITE);
