...
/* JUNOS Start */
#ifdef CONFIG_EX4500
/* Kludge to get EX4500 working */
extern uint mdio_mode_select(uint mode, struct tsec_private * priv);
struct tsec_private *privlist1[MAXCONTROLLERS];
extern void mdio_mode_set (uint8_t mode, int inuse);
static int num_tsecs_tsunami = 0;
...
static struct phy_info *get_phy_info_priv(struct tsec_private *priv);
#endif
/* JUNOS End */
...
/* JUNOS Start */
#ifdef CONFIG_EX45XX
void init_registers_private(volatile tsec_t * regs)
{
	init_registers(regs);
}
#endif
...
static int init_phy(struct eth_device *dev)
{
	...
	/* JUNOS Start */
#ifdef CONFIG_EX4500
	/* ex4500 needs some workarounds due to a secondary phy */
	if (priv->phyaddr) {
	    if (priv->index == 3)
	...
	if (priv1->phyaddr) {  /* 1145, 2nd PHY, 0 not valid in EX4500 */
		mdio_mode_set(0, 1);
		phydev = phy_connect(priv1->bus, priv1->phyaddr, dev, priv1->interface);
		if (phydev == NULL) {
			phydev = NULL;
			printf("%s: No PHY found\n", dev->name);
			mdio_mode_set(0, 0);
...
static int tsec_initialize(bd_t *bis, struct tsec_info_struct *tsec_info)
{
	...
	/* JUNOS Start */
#ifdef CONFIG_EX4500
	priv->index = num_tsecs_tsunami;
	priv1 = (struct tsec_private *)malloc(sizeof(*priv));
	if (NULL == priv1)
	    return 0;
	priv1->regs = tsec_info->regs;
	priv1->phyregs_sgmii = tsec_info->miiregs_sgmii;

	priv1->index = num_tsecs_tsunami;
	priv1->phyaddr = tsec_info1[num_tsecs_tsunami].phyaddr;
	priv1->flags = tsec_info1[num_tsecs_tsunami].flags;
	privlist1[num_tsecs_tsunami++] = priv1;
	...
	/* JUNOS Start */
#ifdef CONFIG_EX4500
	priv1->bus = miiphy_get_dev_by_name(tsec_info->mii_devname);
#endif
	/* JUNOS End */
	dev->iobase = 0;
	dev->priv = priv;
	...
	dev->recv = tsec_recv;
	/* JUNOS Start */
#ifdef CONFIG_TSEC_USE_HWUNIT_IDX
	dev->hw_unit = tsec_info->hw_unit;
#else
	dev->hw_unit = 0;
#endif
	/* JUNOS End */
	...
	/* JUNOS Start */
#ifdef CONFIG_EX4500
	/* Old boxes need some workarounds for tsec */
	svr = get_svr();
