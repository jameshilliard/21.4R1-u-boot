				...
				struct mkimage_params *params)
{
	...
	/* JUNOS Start */
	if (!params->cflag) {
		checksum = crc32 (0, (const unsigned char *)(ptr + 
			sizeof(image_header_t)),
			sbuf->st_size - sizeof(image_header_t));
	} else {
		/* embedded header in firmware offset 0x40 */
		checksum = crc32(0, (const unsigned char *)(ptr + 0x100 - IMG_HEADER_OFFSET),
	...
	if (!params->cflag) {
		 ...
		 */
		image_set_size (hdr, sbuf->st_size - 0x100);
	}
